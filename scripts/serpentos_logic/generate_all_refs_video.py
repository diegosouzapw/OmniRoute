#!/usr/bin/env python3
# =============================================================================
# Autonomous Video Generator & Montage Agent (All 23 Refs - Vertex AI Veo + FFmpeg)
...
async def generate_clip(client, clip, idx, total_count):
    ...
    prompt_text = clip.get("veo_prompt")
    if not prompt_text:
        # Fallback construction from available metadata fields
        prompt_text = ", ".join(filter(None, [
            clip.get("description"),
            clip.get("style"),
            clip.get("camera"),
            clip.get("lighting"),
            clip.get("environment"),
            clip.get("motion"),
            clip.get("ending")
        ]))
        ...
        try:
            op = client.models.generate_videos(
                ...
                config=types.GenerateVideosConfig(
                    ...
                    resolution="720p", # Cost-efficient & fast resolution
...
def compile_final_video(processed_paths):
    print("🎬 Compiling all-refs showreel via FFmpeg...")
    
    temp_lut = os.path.join(OUTPUT_DIR, "temp_kodak_lut_all.cube")
    ...
    output_mp4 = os.path.join(OUTPUT_DIR, "showreel_all_refs_final.mp4")
    total_dur_sec = len(processed_paths) * 6 - (len(processed_paths) - 1) * 0.5
    ...
    print(f"✅ All-Refs Showreel compiled: {output_mp4}")
    return output_mp4

def generate_xml(clip_paths):
    output_xml_path = os.path.join(OUTPUT_DIR, "showreel_all_refs_timeline.xml")
    print("📝 Generating FCP XML timeline...")
    timebase = 24
    
    xmeml = ET.Element("xmeml", version="5")
    sequence = ET.SubElement(xmeml, "sequence", id="sequence-all-refs")
    ET.SubElement(sequence, "name").text = "All_Refs_Showreel_Timeline"
...
async def main():
    print("=== Video Generation & Montage Agent (All 23 Refs) ===")
    ...
    target_clips = data.get("clips", [])
    if not target_clips:
        print("❌ No matching clips found for generation.")
        sys.exit(1)
        
    print(f"Total reference clips in library: {len(target_clips)}")
    ...
    # 2. Run video generation (Batch of 2 at a time)
    batch_size = 2
    generated_paths = []
