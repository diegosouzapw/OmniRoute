...
<div class="flex flex-col gap-6">
  <div>
    <h1 class="text-2xl font-bold">Usage Limits</h1>
    <p class="text-sm text-text-muted mt-1">View and manage provider usage limits.</p>
  </div>

  {#if loading}
    <div class="flex items-center justify-center min-h-[200px]">
      <div class="text-text-muted">Loading limits...</div>
    </div>
  {:else if error}
  ...
  {:else}
    <div class="rounded-lg border border-border/40 bg-bg-subtle/30 px-4 py-3">
      <p class="text-sm text-text-muted">Limits settings loaded. Redirect to Quota page for full management.</p>
    </div>
  {/if}
</div>
