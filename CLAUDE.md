...
- **Runtime**: Node.js ≥22.0.0 <23 || ≥24.0.0 <27, ES Modules. Node remains the default and `engines.node` is authoritative for the published package. Bun `1.3.14` is also supported for the published CLI/server Bun path, using `bun:sqlite` and Bun-compatible child-process spawning; the full test suite and Node-only integrations remain on Node.
- **Bun**: Bun `1.3.14` is pinned as an **exact devDependency** (provisioned through the existing `npm ci` via the lockfile's `@oven/bun-*` platform binaries — no `setup-bun`/ad-hoc install). It runs the existing allow-listed TypeScript gate/generator scripts and the focused `test:bun:db` compatibility suite. Bun-specific runtime changes must preserve the Node driver/fallback path and include a Bun test or an explicit reason why the path is Node-only. After pulling the lockfile change, run `npm install` so `bun` resolves locally (a stale `node_modules` will fail those scripts with `bun: not found`).
