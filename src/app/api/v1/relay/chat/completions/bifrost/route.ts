 ...
 *
 * Falls back to the TypeScript relay route when:
 *   - BIFROST_BASE_URL is unset
 *   - The Go sidecar is unreachable (graceful degradation — the user
 *     gets the slower path, not a 502)
 *   - Request body has features the Go gateway doesn't yet support
 *     (audio, video, image gen, embeddings — call out to the JS path)
 *
 ...
 *
 * @see src/lib/adapters/bifrostAdapter.ts (the long-lived TS fallback)
 * @see docs/operations/bifrost-migration.md
 */
