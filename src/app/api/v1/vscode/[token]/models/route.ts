...
export function getVscodeRawModelDisplayName(model: CatalogModelEntry) {
  ...
  const prefixedDisplayName =
    providerPrefix && !matchesSearch(baseDisplayName, providerPrefix)
  ...
  const selectedReasoningEffort = reasoningEffortValues
    ? getCatalogModelName(model as VscodeCatalogModel)
        .match(/-(max|xhigh|high|medium|low|none)$/i)?.[1]
        ?.toLowerCase()
    : undefined;
...
export function enrichModelForVscode(
  ...
  const capabilityModelId = model.root || model.id || model.name || "";
  const resolvedCapabilities = model.owned_by
    ? getResolvedModelCapabilities({ provider: model.owned_by, model: capabilityModelId })
    : getResolvedModelCapabilities(capabilityModelId);
  const candidateReasoningEffortValues = getReasoningEffortValues(model as VscodeCatalogModel);
  const hasNativeReasoningEffortValues =
    Array.isArray(model.supportsReasoningEffort) || Array.isArray(model.supportedReasoningEfforts);
  const reasoningEffortValues =
    hasNativeReasoningEffortValues || resolvedCapabilities.supportsThinking === true
      ? candidateReasoningEffortValues
      : undefined;
  ...
  const knownCapabilityFields = {
    ...(resolvedCapabilities.supportsTools !== null
      ? { toolCalling: resolvedCapabilities.supportsTools }
      : {}),
    ...(resolvedCapabilities.supportsVision !== null
      ? { vision: resolvedCapabilities.supportsVision }
      : {}),
  };
