...
function getOllamaModelFamily(model: OpenAiCatalogModel, canonicalFamily?: string | null) {
	...
	const modelFamily = tierParsedModel.baseModelId.includes("/")
		? tierParsedModel.baseModelId.split("/").slice(1).join("/")
		: tierParsedModel.baseModelId;
...
function toOllamaTagModel(model: OpenAiCatalogModel) {
	const actualModelId = model.id || model.root || model.name || "unknown";
	...
	return {
		...
		size: 0,
		digest: `omniroute:${actualModelId}`,
