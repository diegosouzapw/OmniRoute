...
function sanitizeComboCapabilities(capabilities: Record<string, boolean> | undefined) {
...
function buildComboShowPayload(combo: ComboCatalogEntry) {
  ...
  const configSchema =
    reasoningEffortValues && defaultReasoningEffort
      ? buildReasoningConfigSchema(reasoningEffortValues, defaultReasoningEffort)
      : undefined;
  const modelCapabilities = sanitizeComboCapabilities(combo.capabilities);
  ...
  if (combo.capabilities?.reasoning) capabilities.push("thinking");
