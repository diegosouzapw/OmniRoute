...
export async function fetchAntigravityDiscoveryModelsCached(
  ...
  const profile = normalizeAntigravityClientProfile(
    asRecord(providerSpecificData).clientProfile
  );
  ...
  const promise = (async () => {
    ...
    for (const discoveryUrl of [
      ...
      try {
        ...
        console.warn(
          `[models] antigravity discovery threw for ${discoveryUrl}: ${message}`
        );
      }
    }
...
export function normalizeDataRobotCatalogResponse(
  ...
  const items = Array.isArray(asRecord(data).data)
  ...
  return items
    .map((value) => {
      ...
      const name =
        toNonEmptyString(item.label) || toNonEmptyString(item.displayName) || model;
  ...
  return items
    .map((value) => {
      ...
      const name =
        ...
        id;
      const ownedBy =
        toNonEmptyString(item.owned_by) ||
        toNonEmptyString(item.provider) ||
        fallbackOwner;
      return { id, name, owned_by: ownedBy };
    })
    .filter(
      (value): value is { id: string; name: string; owned_by: string } =>
        Boolean(value)
    );
}
