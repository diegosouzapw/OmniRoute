...
class VectorStoreImpl implements VectorStore {
  ...
  async searchHybrid(
    ...
    const rrfK = RRF_K;
    const q = liveVecQuantization();
    // The FTS half of the fusion shares this statement with the vector half, so
    // an FTS5 syntax error from raw user text (a bare `-`, `"`, `*`, `col:`)
    // fails the WHOLE hybrid query and discards the vector hits too. Escape it;
    // when nothing searchable survives, pass a term that matches no document so
    // the vector side still returns.
    const ftsMatch = sanitizeFtsQuery(queryText) ?? '""';
