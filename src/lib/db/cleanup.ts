...
export async function cleanupMemoryEntries(): Promise<CleanupResult> {
  ...
  try {
    ...
    console.log(
      `[Cleanup] Deleted ${result.deleted} memories older than ${retentionDays} days`
    );
  } catch (err: unknown) {
...
export function startCleanupScheduler(): void {
  ...
  setTimeout(async () => {
    try {
      ...
      if (totalDeleted > 0) {
        ...
        try { getDbInstance().exec("VACUUM"); console.log("[Cleanup] VACUUM completed."); } catch (e) { console.error("[Cleanup] VACUUM failed:", e); }
      }
    } catch (err) { console.error("[Cleanup] Startup cleanup failed:", err); }
  }, 30_000);
  _cleanupSchedulerTimer = setInterval(async () => {
    ...
    } catch (err) { console.error("[Cleanup] Periodic cleanup failed:", err); }
  }, CLEANUP_INTERVAL_MS);
  if (_cleanupSchedulerTimer && typeof _cleanupSchedulerTimer.unref === "function") _cleanupSchedulerTimer.unref();
  console.log("[Cleanup] Background cleanup scheduler started (every 6 hours).");
}

export function stopCleanupScheduler(): void {
  if (_cleanupSchedulerTimer) { clearInterval(_cleanupSchedulerTimer); _cleanupSchedulerTimer = null; }
}
