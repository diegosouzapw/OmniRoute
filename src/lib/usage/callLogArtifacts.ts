...
export function writeCallArtifact(
  ...
  try {
    ...
    // We use SHA-256 for file checksums, not passwords
     
    const fileChecksum = crypto.createHash("sha256").update(serialized).digest("hex"); // codeql[js/insufficient-password-hash]
