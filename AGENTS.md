...
> **Live counts (v3.8.49)**: providers 250 · MCP tools 104 · MCP scopes 30 · A2A skills 6 ·
...
**104 unique tools** total (`TOTAL_MCP_TOOL_COUNT`, `open-sse/mcp-server/server.ts`): a 42-entry canonical registry (`MCP_TOOLS` in `schemas/tools.ts`, including the six CCR lifecycle tools) **plus** standalone module sets — memory (3), skill (4), agentSkill (3, already canonical), GitHub skill (3), pool (6), gamification (8), plugin (8), notion (6), obsidian (22), and RTK-only compression tools (2). `countUniqueMcpTools()` deduplicates overlapping definitions. 3 transports (stdio / SSE / Streamable HTTP). Scoped auth (30 scopes — see `OMNIROUTE_MCP_SCOPES`), Zod schemas. See [`docs/frameworks/MCP-SERVER.md`](docs/frameworks/MCP-SERVER.md).
