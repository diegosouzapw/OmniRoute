...
- **fix(server):** the standalone server no longer exits immediately under systemd (`Type=simple`, no TTY) — Next's standalone entry calls `startServer()` fire-and-forget, so under systemd's fast, TTY-less execution the process could finish evaluating before `server.listen()` bound, exiting 0 and causing systemd to cycle the service. A startup keep-alive now holds the event loop until the HTTP server reaches `listening` (cleared on a safety timeout if it never binds). ([#3416](https://github.com/diegosouzapw/OmniRoute/issues/3416) — thanks @samuraiIT)
