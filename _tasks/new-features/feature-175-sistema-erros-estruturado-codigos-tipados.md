# Feature 26 - Sistema de erros estruturado com códigos tipados

## Resumo executivo

- Feature 26 priorizada como **P1**, com foco em endurecer a borda do serviço contra abuso, SSRF e chamadas inseguras em /v0/management/_, /api/settings/_, /api/usage/\*.
...
A feature impacta diretamente `/v0/management/*`, `/api/settings/*`, `/api/usage/*`. O objetivo técnico fechado é endurecer a borda do serviço contra abuso, SSRF e chamadas inseguras em /v0/management/_, /api/settings/_, /api/usage/\*.
...
- Métricas acionáveis para operação: `omniroute_feature_26_requests_total`, `omniroute_feature_26_errors_total`, `omniroute_feature_26_latency_ms`.
...
- Telemetria mínima para gate de rollout: `omniroute_feature_26_requests_total`, `omniroute_feature_26_errors_total`, `omniroute_feature_26_latency_ms`.

## Passo a passo de implementação por arquivo

1. Em `docs/openapi.yaml`, introduzir schema/config para a feature com defaults `security.outbound.allowlist.enforced=true`, `security.login.rate_limit_per_min=10`, `security.ssrf.block_private_ranges=true` e validação de tipo/faixa.
2. Em `docs/API_REFERENCE.md`, criar/ajustar constantes, enums e helpers para erros tipados (`INTERNAL_PROXY_ERROR`, `SSRF_BLOCKED`, `AUTH_RATE_LIMITED`).
...
- Instrumentar métricas obrigatórias: `omniroute_feature_26_requests_total`, `omniroute_feature_26_errors_total`, `omniroute_feature_26_latency_ms`.
...
3. Gate para avançar: `omniroute_feature_26_errors_total` <= baseline + 1% e `omniroute_feature_26_latency_ms` <= baseline + 15% por 24h.
...
- Integração: exercer fluxo completo das rotas `/v0/management/*`, `/api/settings/*`, `/api/usage/*` em `tests/security/test-docker-hardening.sh` com mocks realistas de upstream.
...
- Observabilidade ativa com métricas `omniroute_feature_26_requests_total`, `omniroute_feature_26_errors_total`, `omniroute_feature_26_latency_ms` e alertas configurados.
...
- `docs/new_features/feature-85-sistema-de-erros-estruturado-com-codigos-tipados.md`
