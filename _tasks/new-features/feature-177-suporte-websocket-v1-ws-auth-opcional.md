# Feature 24 - Suporte websocket em `/v1/ws` com auth opcional

## Resumo executivo

- Feature 24 priorizada como **P1**, com foco em endurecer a borda do serviço contra abuso, SSRF e chamadas inseguras em /v1/ws.
...
A feature impacta diretamente `/v1/ws`. O objetivo técnico fechado é endurecer a borda do serviço contra abuso, SSRF e chamadas inseguras em /v1/ws.
...
- Métricas acionáveis para operação: `omniroute_feature_24_requests_total`, `omniroute_feature_24_errors_total`, `omniroute_feature_24_latency_ms`.
...
- Defaults operacionais fixados: `security.outbound.allowlist.enforced=true`, `security.login.rate_limit_per_min=10`, `security.ssrf.block_private_ranges=true`, `proxy.ws.auth_optional=true`, `oauth.refresh.min_ttl_seconds=300`.
...
- Telemetria mínima para gate de rollout: `omniroute_feature_24_requests_total`, `omniroute_feature_24_errors_total`, `omniroute_feature_24_latency_ms`, `omniroute_feature_24_ws_open_connections`, `omniroute_feature_24_auth_fail_total`.

## Passo a passo de implementação por arquivo

1. Em `open-sse/handlers/sseParser.js`, introduzir schema/config para a feature com defaults `security.outbound.allowlist.enforced=true`, `security.login.rate_limit_per_min=10`, `security.ssrf.block_private_ranges=true` e validação de tipo/faixa.
2. Em `open-sse/utils/stream.js`, criar/ajustar constantes, enums e helpers para erros tipados (`INTERNAL_PROXY_ERROR`, `SSRF_BLOCKED`, `AUTH_RATE_LIMITED`).
...
5. Em `tests/e2e/navigation.spec.ts` (unitário) e `tests/integration/security-hardening.test.mjs` (integração/contrato), cobrir caminho feliz, erro tipado e regressão do fluxo legado.
...
- Instrumentar métricas obrigatórias: `omniroute_feature_24_requests_total`, `omniroute_feature_24_errors_total`, `omniroute_feature_24_latency_ms`, `omniroute_feature_24_ws_open_connections`, `omniroute_feature_24_auth_fail_total`.
...
3. Gate para avançar: `omniroute_feature_24_errors_total` <= baseline + 1% e `omniroute_feature_24_latency_ms` <= baseline + 15% por 24h.
...
- Unitário: validar regras internas e normalização de entradas no arquivo `tests/e2e/navigation.spec.ts`.
- Integração: exercer fluxo completo das rotas `/v1/ws` em `tests/integration/security-hardening.test.mjs` com mocks realistas de upstream.
...
- Regressão e2e: assegurar não quebra de comportamento existente em `tests/security/test-docker-hardening.sh`.
...
- Observabilidade ativa com métricas `omniroute_feature_24_requests_total`, `omniroute_feature_24_errors_total`, `omniroute_feature_24_latency_ms` e alertas configurados.
...
- `docs/new_features/feature-54-suporte-websocket-v1-ws.md`
- `docs/new_features/feature-67-suporte-websocket-em-v1-ws-com-auth-opcional.md`
