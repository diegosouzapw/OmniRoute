# Feature 47 - Copilot quota snapshots e parsing detalhado

## Resumo executivo

- Feature 47 priorizada como **P1**, com foco em fortalecer sync, ownership de tokens e discovery dinâmico com rastreabilidade em /api/monitoring/health, /api/provider-metrics, /api/usage/analytics.
...
- A semântica de erro passa a ser tipada com códigos como `INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`, `SYNC_TOKEN_INVALID`.
...
A feature impacta diretamente `/api/monitoring/health`, `/api/provider-metrics`, `/api/usage/analytics`. O objetivo técnico fechado é fortalecer sync, ownership de tokens e discovery dinâmico com rastreabilidade em /api/monitoring/health, /api/provider-metrics, /api/usage/analytics.
...
- Métricas acionáveis para operação: `omniroute_feature_47_requests_total`, `omniroute_feature_47_errors_total`, `omniroute_feature_47_latency_ms`.
...
- Defaults operacionais fixados: `sync.share_code.ttl_seconds=900`, `oauth.pool.ownership_mode=strict`, `antigravity.discovery.interval_ms=180000`, `routing.quota.preflight.enabled=true`.
- Headers obrigatórios de diagnóstico: `X-Request-Id`, `Retry-After`, `X-RateLimit-Remaining`, `X-Sync-Version`, `X-Sync-Source`.
- Códigos de erro obrigatórios: `INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`, `SYNC_TOKEN_INVALID`, `SYNC_CONFLICT`.
- Telemetria mínima para gate de rollout: `omniroute_feature_47_requests_total`, `omniroute_feature_47_errors_total`, `omniroute_feature_47_latency_ms`, `omniroute_feature_47_quota_switch_total`.

## Passo a passo de implementação por arquivo

1. Em `src/app/api/monitoring/health/route.js`, introduzir schema/config para a feature com defaults `sync.share_code.ttl_seconds=900`, `oauth.pool.ownership_mode=strict`, `antigravity.discovery.interval_ms=180000` e validação de tipo/faixa.
2. Em `src/lib/usageAnalytics.js`, criar/ajustar constantes, enums e helpers para erros tipados (`INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`, `SYNC_TOKEN_INVALID`).
...
- Erros novos devem coexistir com fallback para códigos genéricos até final da janela (`INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`).
...
- Instrumentar métricas obrigatórias: `omniroute_feature_47_requests_total`, `omniroute_feature_47_errors_total`, `omniroute_feature_47_latency_ms`, `omniroute_feature_47_quota_switch_total`.
...
3. Gate para avançar: `omniroute_feature_47_errors_total` <= baseline + 1% e `omniroute_feature_47_latency_ms` <= baseline + 15% por 24h.
...
- Integração: exercer fluxo completo das rotas `/api/monitoring/health`, `/api/provider-metrics`, `/api/usage/analytics` em `tests/security/test-cloud-openai-compatible.sh` com mocks realistas de upstream.
- Contrato: garantir status/headers/body e códigos de erro (`INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`, `SYNC_TOKEN_INVALID`) em fixtures versionadas.
...
- Erros tipados entregues e documentados, cobrindo ao menos `INTERNAL_PROXY_ERROR`, `QUOTA_EXHAUSTED`, `SYNC_TOKEN_INVALID`.
- Cobertura de testes: unitário + integração + contrato + regressão para caminho feliz e falhas críticas.
- Observabilidade ativa com métricas `omniroute_feature_47_requests_total`, `omniroute_feature_47_errors_total`, `omniroute_feature_47_latency_ms` e alertas configurados.
...
- `docs/new_features/feature-79-copilot-quota-snapshots-parsing-detalhado.md`
