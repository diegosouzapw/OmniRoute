# Feature 07 - Segurança de management (chave, escopo e auditoria)

## Resumo executivo

- Feature 07 priorizada como **P1**, com foco em estabilizar o contrato de management e eliminar divergências entre o plano de controle e o plano de dados em /api/auth/_, /api/providers/validate, /v0/management/_.
...
A feature impacta diretamente `/api/auth/*`, `/api/providers/validate`, `/v0/management/*`. O objetivo técnico fechado é estabilizar o contrato de management e eliminar divergências entre o plano de controle e o plano de dados em /api/auth/_, /api/providers/validate, /v0/management/_.
...
- Métricas acionáveis para operação: `omniroute_feature_07_requests_total`, `omniroute_feature_07_errors_total`, `omniroute_feature_07_latency_ms`.
...
- Telemetria mínima para gate de rollout: `omniroute_feature_07_requests_total`, `omniroute_feature_07_errors_total`, `omniroute_feature_07_latency_ms`.

## Passo a passo de implementação por arquivo

1. Em `SECURITY.md`, introduzir schema/config para a feature com defaults `management.compat.enabled=true`, `management.auth.required=true`, `management.contract.version=v0` e validação de tipo/faixa.
2. Em `src/app/api/auth/login/route.js`, criar/ajustar constantes, enums e helpers para erros tipados (`INTERNAL_PROXY_ERROR`, `MGMT_UNAUTHORIZED`, `MGMT_FORBIDDEN_SCOPE`).
...
- Instrumentar métricas obrigatórias: `omniroute_feature_07_requests_total`, `omniroute_feature_07_errors_total`, `omniroute_feature_07_latency_ms`.
...
3. Gate para avançar: `omniroute_feature_07_errors_total` <= baseline + 1% e `omniroute_feature_07_latency_ms` <= baseline + 15% por 24h.
...
- Integração: exercer fluxo completo das rotas `/api/auth/*`, `/api/providers/validate`, `/v0/management/*` em `tests/e2e/api.spec.ts` com mocks realistas de upstream.
...
- Observabilidade ativa com métricas `omniroute_feature_07_requests_total`, `omniroute_feature_07_errors_total`, `omniroute_feature_07_latency_ms` e alertas configurados.
...
- `docs/new_features/feature-20-seguranca-da-camada-management-chave-escopo-e-auditoria.md`
