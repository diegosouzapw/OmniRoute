...
```bash
curl "http://localhost:30128/api/github-skills?minStars=3&maxResults=50"
```
...
```bash
curl -X POST http://localhost:30128/api/skills/collect/install \
...
if [ -n "$TOOL" ] && [ -n "$SKILL" ]; then
  # 3. Install
  curl -s -X POST http://localhost:30128/api/skills/collect/install \
    -H "Content-Type: application/json" \
