...
curl -X POST http://localhost:20128/api/provider-models \
  ...
  -d '{"provider": "openai", "modelId": "gpt-5.2", "modelName": "GPT-5.2", "capabilities": {"contextWindow": 128000, "supportsVision": true, "supportsTools": true}}'

# List: curl http://localhost:20128/api/provider-models?provider=openai
# Reset config overrides: curl -X POST http://localhost:20128/api/provider-models/reset -H "Content-Type: application/json" -d '{"provider":"openai","modelId":"gpt-5.2"}'
# Remove: curl -X DELETE "http://localhost:20128/api/provider-models?provider=openai&model=gpt-5.2"
```

Or use Dashboard: **Providers → [Provider] → Custom Models**. The model compatibility panel
also edits capabilities such as context window, vision, tool calling, thinking, and xhigh/max
reasoning support; Reset restores the synced or registry baseline without changing visibility.
