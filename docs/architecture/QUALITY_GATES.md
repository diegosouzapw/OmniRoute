...
| `test:vitest:ui` | UI component tests — vitest runner                      | **Blocking** — pre-existing failures explicitly excluded in `vitest.config.ts`; new failures fail the job |
...
- `test:vitest:ui` (now **blocking**) — pre-existing failures are explicitly excluded in `vitest.config.ts` with `// FIXME` comments; new failures fail the job.
