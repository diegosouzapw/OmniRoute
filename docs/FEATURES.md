...
Manage 160+ AI provider connections organized by category: **Free providers** (Qoder, Kiro, Gradient Network), **OAuth providers** (Claude Code, Codex, Cursor, GitHub Copilot, Kimi Coding, Kilo Code, Cline, Amp, Trae, Zed, GitLab Duo, and more), **API Key providers** (OpenAI, Anthropic, Gemini, DeepSeek, Groq, xAI, Mistral, OpenRouter, and 100+ more), **Enterprise Cloud** (Vertex AI, Cloudflare AI, Databricks, Snowflake, AWS SageMaker, IBM WatsonX, OCI Oracle), **Agent Frameworks** (Vercel AI Gateway, Baseten, Inworld, LangGraph, Pydantic AI), **IDE/Coding** (OpenCode, Codestral, Bailian Coding), **Media** (ElevenLabs, Deepgram, ComfyUI, SD WebUI), and **Local/Self-Hosted** (Ollama Cloud, vLLM, LM Studio, Llamafile, NVIDIA Triton). Kiro accounts include credit balance tracking — remaining credits, total allowance, and renewal date visible in Dashboard → Usage.
