...
### فشردهسازی آگاه به کش

برخی از ارائهدهندگان (مانند Anthropic با کش کردن پرامپت) از **کش کردن پرامپت** پشتیبانی میکنند، که به آنها اجازه میدهد بخشهایی از پرامپت را کش کنند تا هزینهها و تأخیر را کاهش دهند. هنگامی که کش فعال است، فشردهسازی تهاجمی در واقع میتواند به عملکرد **آسیب برساند** زیرا توکنهای کش شده را تغییر داده و کش را نامعتبر میکند.
...
فشردهسازی آگاه به کش **همیشه روشن است** — نیازی به پیکربندی نیست. این فقط زمانی فعال میشود که:
...
- **نوبتهای اخیر (۰-۳)**: به صورت کلمهبهکلمه نگهداری میشوند (جزئیات کامل)
- **نوبتهای متوسط (۴-۸)**: فشردهسازی سبک (حذف فضای خالی، پاکسازی قالببندی)
- **نوبتهای قدیمی (۹+)**: فشردهسازی غارنشینی (حذف پرکنندهها، خلاصهسازی)
...
کهنسازی تدریجی برای حالتهای `aggressive` و `ultra` **همیشه روشن است**. این روش به ویژه برای موارد زیر مؤثر است:

- جلسات کدنویسی طولانیمدت
- مکالمات چند روزه
- جریانهای کاری عاملمحور با فراخوانیهای ابزار زیاد
...
> (با حداقل کلمات پاسخ دهید. از تعارفات صرف نظر کنید. از جملات کوتاه استفاده کنید.)
...
حالت خروجی غارنشینی بالا، **مسیر تکسبک قدیمی** است. فاز ۴ آن را به یک کاتالوگ از سبکهای خروجی قابل ترکیب تعمیم داد: `OUTPUT_STYLE_CATALOG` در `open-sse/services/compression/outputStyles/catalog.ts`. هر سبک یک دستورالعمل پرامپت سیستمی است که خود مدل را وادار به تولید خروجی ارزانتر میکند؛ سبکها میتوانند با هم فعال شوند و به ترتیب کاتالوگ تزریق میشوند.

| Style                      | `id`          | کاری که انجام میدهد                                                                                                                                                                                   | زبانهای دستورالعمل                                                  |
| -------------------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Terse prose                | `terse-prose` | کلمات اضافی/حروف تعریف/احتیاط را حذف میکند؛ محتوای فنی را دقیق نگه میدارد. همان متن حالت خروجی `caveman` قدیمی (ارجاع داده شده، نه دوباره تایپ شده).                                                  | en, pt-BR, es, de, fr, it, ru, zh, ja, id, vi                       |
| Less code                  | `less-code`   | نردبان YAGNI: کوچکترین تغییر کارآمد، بدون انتزاعات درخواست نشده.                                                                                                                                      | en, pt-BR, es, de, fr, it, ru, zh, ja, id, vi                       |
| Ponytail (lazy senior dev) | `ponytail`    | "بهترین کد، کدی است که هرگز نوشته نشده است": استفاده مجدد > بازنویسی، علت اصلی > علامت، کوتاهترین تفاوت کارآمد.                                                                                       | en, pt-BR, es, de, fr, it, ru, zh, ja, id, vi                       |
| I have ADHD (action-first) | `i-have-adhd` | اول عمل (دستور/مسیر/قطعه کد قبل از متن)، مراحل شمارهگذاری شده و محدود، یک گام بعدی مشخص، بدون مقدمه/خلاصه/پایانبندی. اقتباس شده از [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd) (MIT). | en, pt-BR, es, de, fr, it, ru, zh, ja, id, vi                       |
| Terse CJK (文言)           | `terse-cjk`   | سبک فوقالعاده مختصر چینی کلاسیک.                                                                                                                                                                      | zh (محدود به منطقه: فقط زمانی ارائه میشود که زبان حل شده `zh` باشد) |

هر سبک سه سطح شدت — `lite`، `full`، `ultra` — را ارائه میدهد و هر سطح
...
`applyOutputStyles()` (`open-sse/services/compression/outputStyles/apply.ts`) انتخاب را
در برابر کاتالوگ حل میکند (شناسههای ناشناخته و سبکهای نامتناسب با منطقه
حذف میشوند، هرگز خطا نیستند)، دستورالعملهای انتخاب شده را به ترتیب کاتالوگ
الحاق میکند، بند مرزها را **یک بار** اضافه میکند، و بلوک را با یک نشانگر
همارزی (`[OmniRoute Output Styles]`) شروع میکند، بنابراین اعمال مجدد یک عملیات
بیاثر است. هنگامی که زبان حل شده (به انتخاب زبان در زیر مراجعه کنید) ترجمه دارد،
دستورالعمل محلی به جای انگلیسی تزریق میشود.

در یک بدنه با `messages`، یک دور زدن محتوا (`shouldBypassCavemanOutputMode()` در
`open-sse/services/compression/outputMode.ts`) سه پیام آخر را بررسی میکند و
در صورتی که با کلمات کلیدی امنیتی، اقدام برگشتناپذیر، شفافسازی یا حساس به ترتیب
آن مطابقت داشته باشند، سبکها را برای کل نوبت نادیده میگیرد. این دور زدن
صرف نظر از تنظیمات **Auto-Clarity Bypass** داشبورد (`cavemanOutputMode.autoClarity`)
اجرا میشود.

هنگامی که دور زدن اجازه عبور نوبت را میدهد، `placeSystemInstruction()` (همان فایل)، که
هرگز یک `messages[0]` جدید ایجاد نمیکند، بلوک را در اولین مورد از موارد زیر که پیدا میکند، قرار میدهد:

1.  یک پیام سیستمی پیشرو با محتوای رشتهای: بلوک پس از متن آن اضافه میشود.
2.  فیلد `system` سطح بالا: بلوک پس از متن یک رشته اضافه میشود، یا
    به عنوان یک بلوک متنی جدید به یک آرایه بلوک محتوا اضافه میشود.
3.  اولین پیام سیستمی بعدی با محتوای رشتهای: بلوک پس از متن آن اضافه میشود.
4.  هیچ یک از موارد بالا: بلوک به یک پیام سیستمی جدید در انتهای `messages` میرود.

در یک بدنه بدون `messages`، بلوک به یک فیلد رشتهای `instructions` اضافه میشود،
یا زمانی که بدنه دارای `input` (یک رشته یا یک آرایه) باشد، به `instructions` تبدیل میشود.
یک بدنه بدون `instructions` و `input` به عنوان `no_messages` نادیده گرفته میشود.
...
سازگاری با نسخههای قبلی: تنظیم ترکیبی `outputMode: "caveman"` قدیمی همچنان کار میکند و به
`terse-prose` نگاشت میشود، که از نظر بایت با تزریق قدیمی در هر زبان قدیمی یکسان است.

انتخاب زبان: با فعال بودن `languageConfig.enabled`، `autoDetect` زبان
آخرین پیام کاربر را انتخاب میکند (همان تشخیصدهنده موتورهای ورودی)؛
خاموش کردن `autoDetect`، `defaultLanguage` را ثابت میکند. خاموش → انگلیسی.

ماتریس سبک × زبان توسط
`tests/unit/compression/output-styles-i18n-matrix.test.ts` ثابت شده است: یک سبک جدید نمیتواند
بدون حداقل یک ترجمه pt-BR (یا یک استثنای ردیابی شده صریح) ارائه شود، و یک
سبک موجود نمیتواند به طور پنهانی یک منطقه را از دست بدهد. برای افزودن یک سبک، به
[EXTENDING_COMPRESSION.md](./EXTENDING_COMPRESSION.md#adding-an-output-style) مراجعه کنید.

### فشردهسازی نتایج ابزار

ماژول `toolResultCompressor.ts` **5 استراتژی فشردهسازی تخصصی** را
...
2.  **فشردهسازی خواندن فایل** — فایلهای بزرگ را کوتاه میکند، سربرگها/واردات را حفظ میکند
...
فشردهسازی نتایج ابزار زمانی که فراخوانی ابزار وجود دارد، **همیشه فعال است**. نیازی به پیکربندی نیست.

### پایپلاین پشتهای

حالت پشتهای **چندین موتور را به صورت متوالی** اجرا میکند — معمولاً ابتدا RTK (۶۰-۹۰٪ صرفهجویی در خروجی ابزار)، سپس Caveman (۳۰٪ صرفهجویی اضافی در متن باقیمانده). این به **۷۸-۹۵٪ صرفهجویی کلی** دست مییابد.
...
- گردشکارهای سنگین ابزار (کدنویسی عاملمحور، تحقیق)
- پردازش دستهای حساس به هزینه
- زمانی که به حداکثر صرفهجویی در توکن نیاز دارید

پیکربندی از طریق ترکیب:
