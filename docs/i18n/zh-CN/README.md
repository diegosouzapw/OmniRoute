...
> 手动叠加各家的免费额度非常痛苦——数十个 SDK、数十种速率限制，而且根本不知道自己实际拥有多少额度。OmniRoute 收录了**分布在 35 个周期性额度池键中的 489 个免费层级条目**，并根据**已公布正数月度预算的 17 个额度池以及五个 Groq 单模型上限**计算 Token 总量，同时按共享额度池进行去重。只有通过区域身份验证后才能使用的额度（目前为 ModelScope）会单独展示；区域身份验证后可额外获得约 600 万 Token，但绝不会计入总量。计算结果会持续显示在仪表板（`/dashboard/free-tiers`）中。

<img src="./docs/diagrams/free-tier-budget.svg" width="100%" alt="OmniRoute 免费层级预算卡片：每月稳定提供约 16.2 亿免费 Token，首月加上注册赠送额度后最高约 22.2 亿；通过一个端点即可使用 35 个已有文档记录的周期性额度池键，覆盖 489 个已收录的免费层级条目。诚实地按额度池去重计算——每个共享额度池仅计算一次，包括 17 个已公布正数月度 Token 预算的周期性额度池，以及五个 Groq 单模型上限；条款风险目录中有 13 家提供者被标记为建议规避，由你自行决定。预算条包括 Mistral 10 亿、Nara 2.1 亿、LLM7 1.5 亿、xKiro 1.5 亿、Groq 3000 万（五个单模型上限）及其他较小额度池；首月注册赠送额度和永久免费的无 Token 上限提供者会单独显示，因此绝不会虚增总量。可在 /dashboard/free-tiers 查看实时已用量/剩余额度。"/>

> 实时 `/dashboard/free-tiers` 页面的动态摘要。完整方法说明（额度池去重、赠送额度层级、提供者条款）：**[docs/reference/FREE_TIERS.md](docs/reference/FREE_TIERS.md)**。
>
> <sub>这些数据每两周都会根据实时目录重新审核，并且**可能上升，也可能下降**——如果某家提供者终止免费层级，数字就会下降；如果新增一家，数字就会上升。我们发布的是目录实际计算出的结果，绝不会采用向上取整的最佳情况。</sub>
...
⭐ 如果 OMNIROUTE 帮你节省了费用并让工作更轻松，请为此仓库加星。

</h3>

[![星标数](https://img.shields.io/github/stars/diegosouzapw/OmniRoute?style=social)](https://github.com/diegosouzapw/OmniRoute)
...
> 一套配置 — `http://localhost:20128/v1` — 即可让**所有** AI IDE 或 CLI 使用免费及低成本模型。
...
<div align="center">
<b>＋ 同样兼容</b> · Agent Deck · Kiro · Command Code · Antigravity · Windsurf · AMP · <b>任何兼容 OpenAI 的工具</b>
</div>

<sub>📖 全部 36 个工具（26 个 CLI Code + 10 个 CLI Agent）的逐工具设置指南 → [`docs/reference/CLI-TOOLS.md`](docs/reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@omniroute/opencode-provider`](https://www.npmjs.com/package/@omniroute/opencode-provider)</sub>
...
凭据按进程注入，并为 Qwen/Gemini 使用一次性的隔离主目录：
...
# 或以交互方式选择提供者和模型，并写入工具自身的配置：
omniroute configure codex          # 也支持：claude opencode qwen aider goose gemini cline continue kilo
```

每条命令都会遵循当前激活的远程上下文（`omniroute connect <host>`），`--dry-run`
可在不执行的情况下预览确切的环境变量和参数，而 `--api-key-env NAME` 可避免将密钥
记录在 shell 历史中。→ [CLI 集成](docs/guides/CLI-INTEGRATIONS.md)

<br/>

<div align="center">

## 🌐 357 个 AI 提供者 — 其中 152 个带有目录免费标记

</div>

> 在规范的聊天、媒体、搜索、本地、云代理和系统集合中，共有 **357 个已注册提供者**，其中 **152 个带有 `hasFree: true` 发现元数据**。聊天模型注册表涵盖 **229 个提供者 / 2,554 个不同的提供者-模型组合 / 1,283 个原始模型 ID**；独立的免费额度目录包含 **491 条按模型划分的记录**、**35 个周期性额度池**，以及 **54 个提供周期性或免密钥永久免费服务的提供者**。这些统计按设计采用不同的分母；相关定义和按额度池去重后的计算方法，请参阅[提供者参考](docs/reference/PROVIDER_REFERENCE.md)和[免费套餐](docs/reference/FREE_TIERS.md)。
...
<sub>……以及另外 330 多个提供者 — 每个图标均从仪表板的提供者目录实时解析。📖 [提供者参考](docs/reference/PROVIDER_REFERENCE.md)</sub>
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="150"><img src="./public/openference.svg" width="42" alt="Openference"/><br/><b>Openference</b><br/><sub>Qwen3.8 27B、Llama 3.2<br/>免费套餐</sub></td>
...
<table>
  ...
  <tr><td nowrap><b>身份验证与安全</b></td><td>OAuth 2.0（PKCE）+ JWT + API 密钥 + MCP 作用域身份验证 · 静态数据使用 AES-256-GCM 加密 · DOMPurify</td></tr>
  ...
  <tr><td nowrap><b>测试</b></td><td>Node.js 测试运行器 + Vitest — 在 5,100 多个跟踪测试文件中包含 <b>39,000 多个静态测试声明</b>（单元测试、集成测试、E2E、安全测试、生态系统测试）</td></tr>
...
<table>
  <tr><th align="left">文档</th><th align="left">描述</th></tr>
  ...
  <tr><td nowrap><b><a href="docs/guides/REMOTE-MODE.md">远程模式</a></b></td><td>通过限定范围的访问令牌，使用笔记本电脑上的 CLI 操控远程 OmniRoute（VPS）</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/reference/ENVIRONMENT.md">环境配置</a></b></td><td>完整的 <code>.env</code> 变量与参考说明</td></tr>
</table>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_GUIDE.md">压缩指南</a></b></td><td>7 种选项的管线：关闭 / 轻量 / 标准 / 激进 / 超级 / RTK / 堆叠</td></tr>
  <tr><td nowrap><b><a href="docs/compression/RTK_COMPRESSION.md">RTK 压缩</a></b></td><td>命令输出压缩、过滤器、信任、验证、原始输出恢复</td></tr>
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_ENGINES.md">压缩引擎</a></b></td><td>Caveman、RTK、堆叠管线、控制面板/API/MCP 接口</td></tr>
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_RULES_FORMAT.md">压缩规则格式</a></b></td><td>用于 Caveman 和 RTK 过滤器的 JSON 规则包模式</td></tr>
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_LANGUAGE_PACKS.md">压缩语言包</a></b></td><td>语言检测与 Caveman 规则包编写</td></tr>
  <tr><td nowrap><b><a href="docs/architecture/RESILIENCE_GUIDE.md">弹性指南</a></b></td><td>断路器、冷却期、队列、防惊群、TLS 欺骗</td></tr>
  ...
  <tr><td nowrap><b><a href="docs/reference/FREE_TIERS.md">免费套餐</a></b></td><td>汇总目录：35 个有文档记录的周期性资源池 / 489 个已编录的免费套餐条目</td></tr>
  <tr><td nowrap><b><a href="docs/guides/FEATURES.md">功能展示</a></b></td><td>带截图的控制面板可视化导览</td></tr>
  <tr><td nowrap><b><a href="docs/architecture/CODEBASE_DOCUMENTATION.md">代码库文档</a></b></td><td>对初学者友好的代码库导览</td></tr>
</table>
...
<table>
  <tr><th align="left">文档</th><th align="left">描述</th></tr>
  <tr><td nowrap><b><a href="docs/reference/API_REFERENCE.md">API 参考</a></b></td><td>所有端点及示例</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="CONTRIBUTING.md">贡献指南</a></b></td><td>开发环境设置与规范</td></tr>
  ...
  <tr><td nowrap><b><a href="CHANGELOG.md">变更日志</a></b></td><td>各版本的完整发布历史</td></tr>
  ...
  <tr><td nowrap><b><a href="docs/ops/COVERAGE_PLAN.md">覆盖率计划</a></b></td><td>面向 5,100 多个跟踪测试文件中 39,000 多项静态测试声明的测试覆盖率策略</td></tr>
</table>
...
<sub>数据定格于实时 <code>release/v3.8.50</code> 分支顶端提交 <code>dafb4ae808</code>，统计了截至 2026-08-24 05:26:03 UTC 的合并记录。分页获取的 GitHub GraphQL 统计数据包含 5,911 个已合并 PR：其中 2,707 个来自仓库所有者，179 个来自 Dependabot，<b>3,025 个外部 PR 来自 535 位不同的贡献者</b>。“变更行数”是 GitHub 统计的新增行数与删除行数之和，其中包括生成的文件、锁定文件、目录、翻译和文档；它表示代码变动量，而非实际编写的代码行数。排名截止位置的并列贡献者均予以保留。</sub>
...
<table>
  <tr>
    ...
    <td align="center" width="160">
      ...
      <sub>🏅 69 次归属于 GitHub 的提交 · 并列第 6</sub>
    </td>
  </tr>
  <tr>
    <td align="center" width="160">
    ...
    <td align="center" width="160">
      ...
      <sub>🏅 51 次归属于 GitHub 的提交 · 并列第 10</sub>
    </td>
    <td align="center" width="160">
...
<sub>于 2026-08-24 06:14:31 UTC 重新核验：仓库 Contributors API 报告了默认分支 <code>release/v3.8.50</code> 上归属于 GitHub 的提交。该 API 返回了 525 个身份（415 名用户、2 个机器人、108 个匿名身份）；此表排除了维护者、机器人和匿名身份，并保留并列排名。它与上方的已合并 PR 排名以及下方基于 Git 元数据统计的 639 人名录均有所区别。</sub>

> 🙏 这些贡献者提供的功能、错误修复和基础设施改进，是 OmniRoute 可靠且功能丰富的**核心组成部分**。每个拉取请求、每个测试用例以及每个 i18n 翻译文件都至关重要。开源项目正是由像他们这样的人共同构建的。
