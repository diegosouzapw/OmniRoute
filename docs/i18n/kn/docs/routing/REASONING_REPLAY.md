...
ಟರ್ನ್ N (ಸಹಾಯಕ ರಚಿಸುತ್ತದೆ):
  ...
  → requiresReasoningReplay(provider, model) ಆಗಿದ್ದರೆ: cacheReasoningFromAssistantMessage()
      ಪ್ರತಿ tool_call.id ಅನ್ನು ಕೀಲಿಯಾಗಿ ಬಳಸಿಕೊಂಡು (ಮೆಮೊರಿ + DB) ಗೆ ಬರೆಯುತ್ತದೆ
  → ಪ್ರತಿಕ್ರಿಯೆಯನ್ನು ಕ್ಲೈಂಟ್ಗೆ ಫಾರ್ವರ್ಡ್ ಮಾಡುತ್ತದೆ (ಅದು ರೀಸನಿಂಗ್ ಅನ್ನು ಉಳಿಸಿಕೊಳ್ಳಬಹುದು ಅಥವಾ ಉಳಿಸಿಕೊಳ್ಳದಿರಬಹುದು)

ಟರ್ನ್ N+1 (ಕ್ಲೈಂಟ್ ಮುಂದಿನ ವಿನಂತಿಯನ್ನು ಕಳುಹಿಸುತ್ತದೆ):
  → ಅನುವಾದಕವು ಪತ್ತೆಹಚ್ಚುತ್ತದೆ: requiresReasoningReplay(provider, model) === true
  → tool_calls ಹೊಂದಿರುವ ಮತ್ತು reasoning_content ಇಲ್ಲದ ಪ್ರತಿ ಸಹಾಯಕ ಸಂದೇಶಕ್ಕಾಗಿ:
      lookupReasoning(toolCalls[0].id) → ಮೆಮೊರಿ → DB
      ಹಿಟ್  → msg.reasoning_content = cached; recordReplay()
      ಮಿಸ್ → msg.reasoning_content = "" (ಹಳೆಯ DeepSeek ಗಾಗಿ ಪರಂಪರೆಯ ಫಾಲ್ಬ್ಯಾಕ್)
  → ಅಪ್ಸ್ಟ್ರೀಮ್ಗೆ ಸುಸಂಗತ ಇತಿಹಾಸ ಕಾಣಿಸುತ್ತದೆ → 400 ಇಲ್ಲ
```

ಸೆರೆಹಿಡಿಯುವಿಕೆಯು `open-sse/handlers/chatCore.ts` ನಲ್ಲಿ ನಡೆಯುತ್ತದೆ (ಎರಡು ಸ್ಥಳಗಳಲ್ಲಿ, ಅಂದರೆ ಎರಡು `cacheReasoningFromAssistantMessage` ಕರೆ ಸ್ಥಳಗಳಲ್ಲಿ). ಸ್ಕೀಮಾ ಕೋರ್ಷನ್ ನಂತರ, ಆದರೆ ಡಿಸ್ಪ್ಯಾಚ್ಗಿಂತ ಮೊದಲು, `open-sse/translator/index.ts` ನಲ್ಲಿ ಮರುಚಲಾವಣೆ ನಡೆಯುತ್ತದೆ.
...
ಇಂಡೆಕ್ಸ್ಗಳು: `expires_at`, `provider`, `model`, `created_at`. `expires_at` ಅನ್ನು Unix epoch ಸೆಕೆಂಡ್ಗಳಾಗಿ ಸಂಗ್ರಹಿಸಲಾಗುತ್ತದೆ; SELECT ಪದರವು `EXPIRES_AT_EPOCH_SQL` ಮೂಲಕ ಹಳೆಯ ಪಠ್ಯ ಮೌಲ್ಯಗಳನ್ನು ಸಾಮಾನ್ಯೀಕರಿಸುತ್ತದೆ.
