...
<img src="./docs/screenshots/MainOmniRoute.png" alt="Панель управління OmniRoute" width="820"/>

<br/>
<br/>

# 🚀 OmniRoute — Безкоштовний AI-шлюз

<img src="./docs/diagrams/readme-hero.svg" width="100%" alt="OmniRoute — Ніколи не припиняйте кодувати. Кожен інструмент ШІ → 359 провайдерів — 150+ безкоштовних — через одну кінцеву точку. Claude Code, Codex, Cursor, Cline, Copilot & Antigravity у БЕЗКОШТОВНІ Claude / GPT / Gemini з автоматичним резервуванням. Стековане стиснення RTK + Caveman економить 15–95% токенів (~89% в середньому) — ніколи не досягайте лімітів. 359 провайдерів ШІ · 150+ безкоштовних рівнів · ~1.62 млрд безкоштовних токенів/міс · 19 стратегій маршрутизації · $0 для початку."/>
...
## 🆓 Працює одразу після встановлення — без ключів, без конфігурації

</div>

<img src="./docs/diagrams/works-zero-config.svg" width="100%" alt="Працює одразу після встановлення — нульова конфігурація. Три кроки: 1. Встановіть — npm i -g omniroute, сервер запускається на localhost:20128. 2. Направте свій інструмент на http://localhost:20128/v1 — будь-який сумісний з OpenAI інструмент (Claude Code, Cursor, Cline). 3. Він відповідає — викличте модель auto для миттєвої відповіді, без ключа API, без реєстрації, без конфігурації. Безключовий провайдер OpenCode Free попередньо підключений до комбінації auto, тому свіжа установка відповідає одразу."/>
...
<sub>Віддаєте перевагу конкретному безкоштовному бекенду? Викликайте `oc/…` (OpenCode Free) безпосередньо. Потім перейдіть на `auto` і дозвольте OmniRoute вибрати.</sub>

<sub>📦 Скопіюйте та вставте скрипти швидкого старту для **Python, Node.js, PHP та cURL** → [`examples/quickstart/`](examples/quickstart/)</sub>
...
<img src="./docs/diagrams/promise-pillars.svg" width="100%" alt="Обіцянка — Одна кінцева точка та 359 провайдерів. Автоматичний відкат продовжує маршрутизацію, поки доступна інша здорова ціль. Шість стовпів: стійкий відкат через 359 провайдерів · до 95% економії токенів на відповідних робочих навантаженнях · $0 для початку зі 150+ безкоштовними рівнями та 54 постійними/безключовими безкоштовними провайдерами · 36 інтеграцій CLI/агентів через одну конфігурацію · сумісність з OpenAI, Claude, Gemini та Responses API на /v1 · виробничі засоби контролю, включаючи автоматичні вимикачі, TLS stealth, інструменти MCP 110, A2A, пам'ять, захисні бар'єри, оцінки та 39 000+ статичних тестових декларацій у 5 100+ відстежуваних тестових файлах."/>
...
<img src="./docs/diagrams/why-pain-fix.svg" width="100%" alt="Чому OmniRoute — припиніть жонглювати 10 панелями, мертвими ключами API та несподіваними рахунками. Десять щоденних проблем проти рішень: квота закінчується невикористаною → максимізуйте підписки; обмеження швидкості під час кодування → 4-рівневий автоматичний відкат (Підписка → API → Дешевий → Безкоштовний); вихідні дані інструментів спалюють токени → стиснення RTK + Caveman (15–95%); дорогі API → маршрутизація, оптимізована за вартістю; кожен інструмент має власне налаштування → одна кінцева точка, одна панель; AI заблоковано → 3-рівневий проксі + TLS stealth; мертві ключі → 3-рівнева стійкість (автоматичні вимикачі, охолодження ключа, блокування моделі); команда ділиться однією підпискою → пули ключів зі справедливими квотами; підказки через чиюсь хмару → локальний пріоритет з ключами, зашифрованими AES-256-GCM; відсутність видимості витрат → жива аналітика (використання, квота, економія, затримка p95)."/>

<div align="center">

<img src="./docs/diagrams/tier-cascade.svg" width="100%" alt="Потік запитів OmniRoute: ваше IDE або CLI (Claude Code, Cursor, Cline…) викликає одну локальну кінцеву точку (http://localhost:20128/v1); інтелектуальний маршрутизатор OmniRoute (стиснення RTK + Caveman, 19 стратегій маршрутизації, автоматичні вимикачі, TLS stealth, MCP, A2A, захисні бар'єри) може відкочуватися через 4 рівні провайдерів, поки залишається відповідна здорова ціль — Рівень 1 Підписка, Рівень 2 Ключ API, Рівень 3 Дешевий та Рівень 4 Безкоштовний."/>
...
<img src="./docs/diagrams/comparison-table.svg" width="100%" alt="Що відрізняє OmniRoute — застарілий знімок функцій у порівнянні з 9router, OpenRouter, CLIProxyAPI та LiteLLM за 13 можливостями. OmniRoute: 359 провайдерів, понад 150 вбудованих безкоштовних рівнів, 19 стратегій маршрутизації, 12-рушійна компресія токенів, вбудований MCP-сервер зі 110 інструментами, протокол агента A2A, постійна пам'ять, захисні механізми, хмарні агенти, приховування відбитків TLS, Desktop/Termux/PWA та 42 локалізовані інтерфейси користувача. OmniRoute має ліцензію MIT і може бути самостійно розміщений. Можливості та кількість конкурентів можуть змінюватися; дивіться пов'язану методологію."/>

<sub>📊 Повна методологія та деталі функцій у порівнянні з 9router, OpenRouter, CLIProxyAPI та LiteLLM → [`docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md`](docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md)</sub>
...
<table>
  ...
  <tr><td nowrap><b>База даних</b></td><td>better-sqlite3 (SQLite, WAL journaling) + LowDB (JSON legacy) — 122 доменних модулі, 182 міграції</td></tr>
  <tr><td nowrap><b>Пам'ять</b></td><td>SQLite FTS5 повнотекстовий пошук + int8-квантовані векторні вбудовування, типізований розпад</td></tr>
  ...
  <tr><td nowrap><b>Потокова передача</b></td><td>Server-Sent Events (SSE) + WebSocket bridge (<code>/v1/ws</code>)</td></tr>
  <tr><td nowrap><b>Стиснення</b></td><td>12-рушійний конвеєр — RTK, Caveman, LLMLingua-2 (MobileBERT ONNX), GCF, OmniGlyph</td></tr>
  <tr><td nowrap><b>Автентифікація та безпека</b></td><td>OAuth 2.0 (PKCE) + JWT + API Keys + MCP scoped auth · AES-256-GCM у стані спокою · DOMPurify</td></tr>
  <tr><td nowrap><b>Невидимість</b></td><td>wreq-js — імітація відбитків JA3 / JA4 TLS, 3-рівневий проксі</td></tr>
  <tr><td nowrap><b>Стійкість</b></td><td>Автоматичний вимикач, експоненційна затримка, захист від "громового стада", самовідновлення авто-комбо</td></tr>
  <tr><td nowrap><b>Логування</b></td><td>pino — структуровані JSON-логи з контекстом запиту</td></tr>
  <tr><td nowrap><b>Тестування</b></td><td>Node.js test runner + Vitest — <b>39 000+ статичних оголошень тестів</b> у 5 100+ відстежуваних тестових файлах (модульні, інтеграційні, E2E, безпека, екосистема)</td></tr>
  <tr><td nowrap><b>Платформи</b></td><td>Десктоп (Electron) · Android (Termux) · PWA (будь-який браузер)</td></tr>
  <tr><td nowrap><b>CI/CD</b></td><td>GitHub Actions — автоматична публікація npm + Docker Hub при релізі</td></tr>
