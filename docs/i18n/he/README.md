...
<img src="./docs/diagrams/readme-hero.svg" width="100%" alt="OmniRoute — לעולם אל תפסיק לקודד. כל כלי AI ← 359 ספקים — 150+ חינמיים — דרך נקודת קצה אחת. Claude Code, Codex, Cursor, Cline, Copilot & Antigravity לתוך Claude / GPT / Gemini בחינם עם גיבוי אוטומטי. דחיסת RTK + Caveman מוערמת חוסכת 15–95% אסימונים (~89% בממוצע) — לעולם אל תגיע למגבלות. 359 ספקי AI · 150+ שכבות חינמיות · ~1.62 מיליארד אסימונים חינם לחודש · 19 אסטרטגיות ניתוב · $0 להתחלה."/>
...
## 🆓 עובד ברגע שמתקינים אותו — ללא מפתחות, ללא הגדרה

</div>

<img src="./docs/diagrams/works-zero-config.svg" width="100%" alt="עובד ברגע שמתקינים אותו — אפס הגדרות. שלושה שלבים: 1. התקנה — npm i -g omniroute, השרת עולה ב-localhost:20128. 2. כוון את הכלי שלך ל-http://localhost:20128/v1 — כל כלי תואם OpenAI (Claude Code, Cursor, Cline). 3. הוא עונה — קרא למודל auto לתשובה מיידית, ללא מפתח API, ללא הרשמה, ללא הגדרה. הספק ללא מפתח OpenCode Free מחובר מראש לשילוב ה-auto, כך שהתקנה חדשה מגיבה ישר מהקופסה."/>

```bash
# התקנה חדשה, אפס אישורים — auto כבר עובד:
curl http://localhost:20128/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"auto","messages":[{"role":"user","content":"Hello!"}]}'
```

<sub>מעדיף/ה בקר קצה חינמי ספציפי? קרא ל-`oc/…` (OpenCode Free) ישירות. לאחר מכן, עבור ל-`auto` ותן ל-OmniRoute לבחור.</sub>

<sub>📦 סקריפטים להתחלה מהירה בהעתק-הדבק עבור **Python, Node.js, PHP, ו-cURL** → [`examples/quickstart/`](examples/quickstart/)</sub>
...
<img src="./docs/diagrams/promise-pillars.svg" width="100%" alt="ההבטחה — נקודת קצה אחת ו-359 ספקים. גיבוי אוטומטי ממשיך לנתב כל עוד יעד בריא אחר זמין. שישה עמודי תווך: גיבוי עמיד על פני 359 ספקים · עד 95% חיסכון באסימונים בעומסי עבודה מתאימים · $0 להתחלה עם 150+ שכבות חינמיות ו-54 ספקים חינמיים קבועים/ללא מפתח · 36 אינטגרציות CLI/סוכן באמצעות הגדרה אחת · תאימות ל-OpenAI, Claude, Gemini ו-Responses API ב-/v1 · בקרות ייצור הכוללות מפסקי זרם, הסוואת TLS, כלי MCP 110, A2A, זיכרון, מעקות בטיחות, הערכות ו-39,000+ הצהרות בדיקה סטטיות על פני 5,100+ קבצי בדיקה במעקב."/>
...
<img src="./docs/diagrams/why-pain-fix.svg" width="100%" alt="למה OmniRoute — הפסק/י להתמודד עם 10 לוחות מחוונים, מפתחות API מתים וחשבונות מפתיעים. עשרה כאבים יומיומיים לעומת פתרונות: מכסה פגה ללא שימוש ← למקסם מנויים; מגבלות קצב באמצע קידוד ← גיבוי אוטומטי 4-שכבתי (מנוי ← API ← זול ← חינם); פלט כלי ששורף אסימונים ← דחיסת RTK + Caveman (15–95%); ממשקי API יקרים ← ניתוב מותאם עלויות; לכל כלי הגדרה משלו ← נקודת קצה אחת, לוח מחוונים אחד; AI חסום ← פרוקסי 3-שכבתי + הסוואת TLS; מפתחות מתים ← עמידות 3-שכבתית (מפסקי זרם, צינון מפתח, נעילת מודל); צוות משתף מנוי אחד ← מאגרי מפתחות עם מכסות חלוקה הוגנת; הנחיות דרך ענן של מישהו ← מקומי תחילה עם מפתחות מוצפנים ב-AES-256-GCM; אין נראות הוצאות ← אנליטיקה חיה (שימוש, מכסה, חיסכון, חביון p95)."/>

<div align="center">

<img src="./docs/diagrams/tier-cascade.svg" width="100%" alt="זרימת בקשות OmniRoute: ה-IDE או ה-CLI שלך (Claude Code, Cursor, Cline…) קורא לנקודת קצה מקומית אחת (http://localhost:20128/v1); הנתב החכם של OmniRoute (דחיסת RTK + Caveman, 19 אסטרטגיות ניתוב, מפסקי זרם, הסוואת TLS, MCP, A2A, מעקות בטיחות) יכול לחזור אחורה על פני 4 שכבות ספקים כל עוד יעד בריא זמין — שכבה 1 מנוי, שכבה 2 מפתח API, שכבה 3 זול ושכבה 4 חינם."/>
...
<img src="./docs/diagrams/comparison-table.svg" width="100%" alt="מה מייחד את OmniRoute — תמונת מצב תכונות מיושנת מול 9router, OpenRouter, CLIProxyAPI ו-LiteLLM על פני 13 יכולות. OmniRoute: 359 ספקים, 150+ שכבות חינמיות מובנות, 19 אסטרטגיות ניתוב, דחיסת אסימונים ב-12 מנועים, שרת MCP מובנה עם 110 כלים, פרוטוקול סוכן A2A, זיכרון מתמשך, מנגנוני הגנה, סוכני ענן, הסוואת טביעת אצבע TLS, Desktop/Termux/PWA ו-42 שפות ממשק משתמש בינלאומיות. OmniRoute ברישיון MIT וניתן לאירוח עצמי. יכולות וספירות המתחרים עשויות להשתנות; ראה את המתודולוגיה המקושרת."/>

<sub>📊 מתודולוגיה מלאה ופרטי תכונות מול 9router, OpenRouter, CLIProxyAPI ו-LiteLLM → [`docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md`](docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md)</sub>
...
<table>
  ...
  <tr><td nowrap><b>שפה</b></td><td>TypeScript 6.0 — <b>100% TypeScript</b> ב-<code>src/</code> וב-<code>open-sse/</code> (אפס <code>any</code> בליבה מאז v2.0)</td></tr>
  <tr><td nowrap><b>פריימוורק</b></td><td>Next.js 16 + React 19 + Tailwind CSS 4</td></tr>
  <tr><td nowrap><b>מסד נתונים</b></td><td>better-sqlite3 (SQLite, יומן WAL) + LowDB (JSON מדור קודם) — 122 מודולי דומיין, 182 מיגרציות</td></tr>
  ...
  <tr><td nowrap><b>הזרמה</b></td><td>אירועים שנשלחו מהשרת (SSE) + גשר WebSocket (<code>/v1/ws</code>)</td></tr>
  <tr><td nowrap><b>דחיסה</b></td><td>צינור 12 מנועים — RTK, Caveman, LLMLingua-2 (MobileBERT ONNX), GCF, OmniGlyph</td></tr>
  <tr><td nowrap><b>אימות ואבטחה</b></td><td>OAuth 2.0 (PKCE) + JWT + מפתחות API + אימות מוגבל MCP · AES-256-GCM במנוחה · DOMPurify</td></tr>
  <tr><td nowrap><b>התגנבות</b></td><td>wreq-js — התחזות טביעת אצבע JA3 / JA4 TLS, פרוקסי 3 רמות</td></tr>
  <tr><td nowrap><b>חוסן</b></td><td>מפסק זרם, נסיגה אקספוננציאלית, מניעת "עדר רועם", ריפוי עצמי אוטומטי משולב</td></tr>
  <tr><td nowrap><b>רישום יומן</b></td><td>pino — יומני JSON מובנים עם הקשר בקשה</td></tr>
  <tr><td nowrap><b>בדיקות</b></td><td>Node.js test runner + Vitest — <b>39,000+ הצהרות בדיקה סטטיות</b> על פני 5,100+ קבצי בדיקה במעקב (יחידה, אינטגרציה, E2E, אבטחה, אקוסיסטם)</td></tr>
  <tr><td nowrap><b>פלטפורמות</b></td><td>מחשב שולחני (Electron) · אנדרואיד (Termux) · PWA (כל דפדפן)</td></tr>
  <tr><td nowrap><b>CI/CD</b></td><td>GitHub Actions — פרסום npm אוטומטי + Docker Hub בשחרור גרסה</td></tr>
