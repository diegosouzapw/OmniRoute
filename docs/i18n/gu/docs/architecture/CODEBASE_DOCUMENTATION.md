...
ડિફૉલ્ટ HTTP પોર્ટ: **`20128`** (API અને dashboard એક જ પ્રોસેસનો ઉપયોગ કરે છે). ડેટા
ડિરેક્ટરી `DATA_DIR` env var છે, જેનું ડિફૉલ્ટ મૂલ્ય `~/.omniroute/` છે.
...
App Router ડૅશબોર્ડ UI અને જાહેર/વ્યવસ્થાપન HTTP API બંને ઉપલબ્ધ કરાવે છે.
અહીં **કોઈ ગ્લોબલ middleware નથી** — ઇન્ટરસેપ્શન દરેક રૂટ પર અલગથી કરવામાં આવે છે.
...
`translator`, `usage`, `webhooks`, ઉપરાંત રૂટ `page.tsx`, `HomePageClient.tsx`,
`BootstrapBanner.tsx`.
...
9Router અને CLIProxyAPI ઇન્સ્ટૉલ કરવા, શરૂ કરવા, રોકવા અને મૉનિટર કરવા માટેના રૂટ્સ.
તમામ પાથને **LOCAL_ONLY** તરીકે વર્ગીકૃત કરવામાં આવ્યા છે (માત્ર લૂપબૅક, કડક નિયમ #17), કારણ કે તેઓ
`npm install` ચલાવી શકે છે અને ચાઇલ્ડ પ્રોસેસિસ સ્પૉન કરી શકે છે.
...
અનુરૂપ ડૅશબોર્ડ UI:
`src/app/(dashboard)/dashboard/providers/services/` — બે-ટૅબનું પૃષ્ઠ (CLIProxyAPI + 9Router).
9Router એમ્બેડેડ UI માટે રિવર્સ પ્રોક્સી:
...
`v1beta/` એ Gemini-શૈલીનું સુસંગતતા સપાટીસ્તર છે (એક પાતળું રૅપર, જે તેનું
સમાન `open-sse/handlers/` પાઇપલાઇનમાં રૂપાંતર કરે છે).

### 3.2 `src/lib/` — મુખ્ય લાઇબ્રેરીઓ

હંમેશાં ડેટા, સિંક, OAuth, સ્કિલ, મેમરી વગેરેને આ મૉડ્યુલો મારફતે ઇમ્પોર્ટ કરો.
કોષ્ટક વાસ્તવિક ડિરેક્ટરીઓ અને નોંધપાત્ર ટોચ-સ્તરની ફાઇલોને જૂથબદ્ધ કરે છે.
...
સિંગલટન SQLite ડેટાબેઝ (`core.ts` માં `getDbInstance()`, WAL જર્નલિંગ).
**રૂટ્સ અથવા હેન્ડલર્સમાં ક્યારેય કાચી SQL લખશો નહીં** — આ મોડ્યુલો મારફતે જ કાર્ય કરો.

![ડેટાબેઝ સ્કીમાનો સંક્ષિપ્ત પરિચય (પસંદ કરેલ મુખ્ય કોષ્ટકો)](../diagrams/exported/db-schema-overview.svg)

> સ્રોત: [diagrams/db-schema-overview.mmd](../diagrams/db-schema-overview.mmd)

ડોમેન મોડ્યુલો (દરેક એક અથવા વધુ કોષ્ટકોની જવાબદારી ધરાવે છે): `apiKeys.ts`, `backup.ts`,
...
`migrations/` માં 168 વર્ઝનવાળી `.sql` ફાઇલો (આઇડેમ્પોટન્ટ, ટ્રાન્ઝેક્શનલ) છે અને
બૂટ સમયે `migrationRunner.ts` દ્વારા તેને ચલાવવામાં આવે છે.
...
`webhooks` (ઉપરાંત મેમરી શોધ માટેનાં FTS5 વર્ચ્યુઅલ કોષ્ટકો).
...
- `constants/` — `providers.ts` (Zod દ્વારા માન્ય કરાયેલ પ્રદાતા સૂચિ), `models.ts`,
  `modelSpecs.ts`, `modelCompat.ts`, `pricing.ts`, `cliTools.ts`,
  `cliCompatProviders.ts`, `routingStrategies.ts`, `comboConfigMode.ts`,
  `headers.ts`, `upstreamHeaders.ts` (પ્રતિબંધિત સૂચિ), `mcpScopes.ts`,
...
- `validation/` — `schemas.ts` (~80 Zod સ્કીમા), `compressionConfigSchemas.ts`,
...
- `utils/` — `circuitBreaker.ts`, `apiAuth.ts`, `apiKey.ts`, `apiKeyPolicy.ts`,
  ...
  `a11yAudit.ts`, ઉપરાંત `services/`, `network/`,
  `middleware/`, `schemas/`, `hooks/`, `components/` હેઠળના ડૅશબોર્ડ હુક્સ/ઘટકો.

---

## 4. `open-sse/` — સ્ટ્રીમિંગ એન્જિન વર્કસ્પેસ

`@omniroute/open-sse` તરીકે પ્રકાશિત થયેલ અલગ npm વર્કસ્પેસ. તે વિનંતી
પ્રક્રિયાકરણ, એક્ઝિક્યુટર્સ, ટ્રાન્સલેટર્સ, સેવાઓ, ટ્રાન્સફોર્મર અને MCP સર્વરનું સંચાલન કરે છે.
...
| હેન્ડલર                 | હેતુ                                                                     |
| ----------------------- | ------------------------------------------------------------------------ |
| `chatCore.ts`           | મુખ્ય ચેટ પાઇપલાઇન (કૅશ, દર મર્યાદા, કોમ્બો રૂટિંગ, એક્ઝિક્યુટર ડિસ્પેચ) |
| `responsesHandler.ts`   | OpenAI Responses API પ્રવેશ બિંદુ                                        |
...
| `usageExtractor.ts`     | અપસ્ટ્રીમ સ્ટ્રીમ્સમાંથી ટોકનની સંખ્યા મેળવવી                            |
| `responseSanitizer.ts`  | પ્રોવાઇડર-વિશિષ્ટ બિનજરૂરી સામગ્રી દૂર કરવી                              |
| `responseTranslator.ts` | પ્રોવાઇડર પ્રતિસાદ અને ટ્રાન્સલેટર સ્તર વચ્ચેનો જોડાણકારી ભાગ            |
...
(સહિયારો ઓળખ સહાયક) અને `index.ts` (રજિસ્ટ્રી).
...
> `src/shared/constants/providers.ts`માં છે.

### 4.3 `open-sse/translator/`

હબ-એન્ડ-સ્પોક અનુવાદ (OpenAI હબ છે).

- **9 વિનંતી ટ્રાન્સલેટર્સ** (`translator/request/`):
  `antigravity-to-openai`, `claude-to-gemini`, `claude-to-openai`,
  `gemini-to-openai`, `openai-responses`, `openai-to-claude`,
  `openai-to-cursor`, `openai-to-gemini`, `openai-to-kiro`.
- **9 પ્રતિસાદ ટ્રાન્સલેટર્સ** (`translator/response/`):
...
- ટોચના સ્તરે: `bootstrap.ts`, `formats.ts`, `registry.ts`, `index.ts`.

### 4.4 `open-sse/transformer/`

- `responsesTransformer.ts` — `TransformStream`-આધારિત Responses API ↔ Chat
  Completions કન્વર્ટર (`responses/` રૂટ કૅચ-ઑલ દ્વારા ઉપયોગમાં લેવાય છે).

### 4.5 `open-sse/services/`

મુખ્ય વિશેષતાઓ (સંપૂર્ણ સૂચિ `open-sse/services/` હેઠળ):
...
| કમ્પ્રેશન           | `compression/` — સંપૂર્ણ કમ્પ્રેશન એન્જિનનું વાયરિંગ                                                                                                                                                                                                     |
...
- `server.ts`માં **110 અનન્ય ટૂલ્સ** જોડાયેલા છે (`schemas/tools.ts`માં 45 પ્રમાણભૂત +
  મેમરી, કૌશલ્યો, GitHub-કૌશલ્યો, પૂલ, ગેમિફિકેશન, પ્લગઇન, Notion, Obsidian,
  લોકલ-કોર્પસ અને કમ્પ્રેશન મૉડ્યુલો — યુનિયનની ગણતરી `countUniqueMcpTools` દ્વારા થાય છે).
- **3 ટ્રાન્સપોર્ટ્સ**: stdio, HTTP Streamable, SSE.
- રનટાઇમ પર **33 સ્કોપ્સ** લાગુ કરવામાં આવે છે — મૂળ સૂચિ `src/shared/constants/mcpScopes.ts`માં છે; સંપૂર્ણ સમૂહ દરેક ટૂલ મૉડ્યુલ દ્વારા જાહેર કરાયેલા સ્કોપ્સનું યુનિયન છે.
...
- ફાઇલો: `server.ts`, `index.ts`, `httpTransport.ts`, `audit.ts`, `scopeEnforcement.ts`,
  ...
  તેમજ `__tests__/` હેઠળના પરીક્ષણો.
...
ઍડેપ્ટરો (`azureAi.ts`, `bedrock.ts`, `datarobot.ts`, `glmProvider.ts`,
`maritalk.ts`, `oci.ts`, `petals.ts`, `runway.ts`, `sap.ts`, `watsonx.ts`,
`ollamaModels.ts`, `errorConfig.ts`, `constants.ts`, `registryUtils.ts`).

### 4.8 `open-sse/utils/`

સ્ટ્રીમિંગ પ્રિમિટિવ્સ અને પ્રોવાઇડર સહાયકો: `stream.ts`, `streamHandler.ts`,
...
વર્કસ્પેસ રૂટ પર પાંચ npm સ્ક્રિપ્ટ્સ: `electron:dev`, `electron:build`,
`electron:build:{win,mac,linux}`, `electron:smoke:packaged`. સ્વચાલિત અપડેટ
`electron-updater` મારફતે થાય છે, જે GitHub રિલીઝ ફીડ તરફ નિર્દેશ કરે છે.
...
`package.json` → `bin`માં બે બાઇનરી ઉપલબ્ધ કરાવવામાં આવી છે:
...
![વિનંતી પાઇપલાઇન (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
| પદ્ધતિ                  | વ્યાપ                       | સ્થાન                                                                                                           |
| ----------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------- |
| પ્રોવાઇડર સર્કિટ બ્રેકર | સમગ્ર પ્રોવાઇડર             | `src/shared/utils/circuitBreaker.ts`, `domain_circuit_breakers`માં સંગ્રહિત                                     |
| કનેક્શન કૂલડાઉન         | એક એકાઉન્ટ/કી               | `src/sse/services/auth.ts`માં `markAccountUnavailable()`; `accountFallback.checkFallbackError()` દ્વારા વપરાયેલ |
| મોડેલ લૉકઆઉટ            | પ્રોવાઇડર + કનેક્શન + મોડેલ | `open-sse/services/accountFallback.ts`, `domain_lockout_state`માં સંગ્રહિત                                      |

[RESILIENCE_GUIDE.md](./RESILIENCE_GUIDE.md) અને
[CLAUDE.md](../../CLAUDE.md)માં સમર્પિત વિભાગ જુઓ.
...
2. જો કસ્ટમ લોજિક જરૂરી હોય, તો `open-sse/executors/` માં executor ઉમેરો
   (`BaseExecutor` ને વિસ્તારો).
3. જો તે OpenAI ફોર્મેટનો ઉપયોગ કરતું ન હોય, તો `open-sse/translator/` માં translator ઉમેરો.
4. જો OAuth-આધારિત હોય, તો `src/lib/oauth/providers/` અને
   `src/lib/oauth/services/` હેઠળ config ઉમેરો.
5. `open-sse/config/providerRegistry.ts` માં models નોંધાવો (અથવા `open-sse/config/`
   હેઠળની ફોર્મેટ-વિશિષ્ટ registry માં).
...
4. જો તે માત્ર management માટે હોય: path ને `src/shared/constants/publicApiRoutes.ts`
   માં ઉમેરો (public API સપાટી માટેની denylist).
...
3. જો નવા tables હોય: `src/lib/db/migrations/` હેઠળ ક્રમિક ક્રમાંકવાળું,
   idempotent અને transactional migration ઉમેરો.
...
1. `open-sse/mcp-server/tools/` હેઠળ tool definition ઉમેરો (અથવા
   `open-sse/mcp-server/schemas/tools.ts` ને વિસ્તારો).
...
5. [MCP-SERVER.md](../frameworks/MCP-SERVER.md) અપડેટ કરો.

### નવી A2A skill ઉમેરો

[A2A-SERVER.md § નવી Skill ઉમેરવી](../frameworks/A2A-SERVER.md) જુઓ. Skills
`src/lib/a2a/skills/` માં રહે છે અને A2A task manager મારફતે નોંધાય છે.
...
- **Code style**: 2-space indent, double quotes, 100 char width, semicolons,
  `es5` trailing commas — `lint-staged` મારફતે Prettier દ્વારા અમલમાં મૂકાય છે.
- **Imports**: external → internal (`@/`, `@omniroute/open-sse`) → relative.
- **Naming**: files `camelCase` અથવા `kebab-case`, components `PascalCase`,
  constants `UPPER_SNAKE`.
- **ESLint**: `no-eval`, `no-implied-eval`, `no-new-func` = દરેક જગ્યાએ `error`;
  `no-explicit-any` = `open-sse/` અને `tests/` માં `warn`, અન્યત્ર error.
- **TypeScript**: `strict: false` (legacy અભિગમ). Cross-module boundaries માટે
  inference કરતાં explicit types ને પ્રાધાન્ય આપો.
- **Database**: routes અથવા handlers માં ક્યારેય raw SQL લખશો નહીં — હંમેશાં
  `src/lib/db/` modules મારફતે જ કાર્ય કરો. ક્યારેય barrel-import કરશો નહીં — નિર્દિષ્ટ `src/lib/db/*` modules નો સીધો ઉપયોગ કરો.
- **DB-entity typing (#3512)**: DB table ના row shape ને લખતું અથવા વાંચતું function,
  તે table ના columns ને 1:1 પ્રતિબિંબિત કરતું named TS interface સ્વીકારવું/પરત કરવું
  જોઈએ, call site પર `any` અથવા inline anonymous type નહીં. Interface ને function ની
  બાજુમાં રાખો (દા.ત. `saveRequestUsage` ની ઉપર `src/lib/usage/usageHistory.ts` માં
  `export interface UsageEntry`), જ્યારે વિવિધ writers row ને ક્રમિક રીતે ભરે
  ત્યારે અલગ-अलग fields ને optional/nullable રાખો, અને callers પ્રમાણે જે field નું
  shape બદલાય તેના માટે `any` કરતાં `unknown` ને પ્રાધાન્ય આપો (field પર તેનું
  documentation આપો, દા.ત. `UsageEntry.tokens` raw provider-shaped usage અને
  normalized shape બંને સ્વીકારે છે). આ રીતે કોઈ file નો `any` count શૂન્ય થાય પછી,
  તેને `check:any-budget:t11` allowlist (`scripts/check/check-t11-any-budget.mjs`,
  `maxAny: 0`) માં ઉમેરો જેથી તેમાં regression ન થઈ શકે. આ first-slice convention છે —
  વ્યાપક "કોઈ anonymous `any` નહીં" cleanup બાકીના codebase માં પુનરાવર્તિત રીતે
  કરવામાં આવે છે.
- **Errors**: નિર્દિષ્ટ error types સાથે try/catch નો ઉપયોગ કરો, pino context સાથે log કરો. SSE streams માં
  errors ને ક્યારેય મૌનપણે અવગણશો નહીં; cleanup માટે abort signals નો ઉપયોગ કરો.
- **Security**: ક્યારેય `eval()` / `new Function()` / implied eval નો ઉપયોગ કરશો નહીં. બધા
  inputs ને Zod વડે validate કરો. સંગ્રહિત credentials ને encrypt કરો (AES-256-GCM).
  `src/shared/constants/upstreamHeaders.ts` denylist ને sanitize/validation
  layer સાથે સુસંગત રાખો.
- **Commits**: Conventional Commits — `feat(scope): subject`. માન્ય scopes:
  `db`, `sse`, `oauth`, `dashboard`, `api`, `cli`, `docker`, `ci`, `mcp`,
  `a2a`, `memory`, `skills`.
- **Branches**: prefixes `feat/`, `fix/`, `refactor/`, `docs/`, `test/`,
  `chore/`. `main` પર ક્યારેય સીધું commit કરશો નહીં.
- **Husky**: pre-commit વખતે `lint-staged` + `check:docs-sync` +
  `check:any-budget:t11` ચાલે છે; pre-push વખતે `check:any-budget:t11` + `check:tracked-artifacts` ચાલે છે (ઝડપી gates; `test:unit` ને બાકાત રાખે છે).
...
4. સીધા `main`માં ક્યારેય કમિટ ન કરો.
...
- [ARCHITECTURE.md](./ARCHITECTURE.md) — ઉચ્ચ-સ્તરીય આર્કિટેક્ચર અને મોડ્યુલની
  જવાબદારીઓ.
- [API_REFERENCE.md](../reference/API_REFERENCE.md) — સાર્વજનિક + મેનેજમેન્ટ API સંદર્ભ.
- [FEATURES.md](../guides/FEATURES.md) — સુવિધા મેટ્રિક્સ અને વર્ઝનની મુખ્ય વિશેષતાઓ.
- [RESILIENCE_GUIDE.md](./RESILIENCE_GUIDE.md) — સર્કિટ બ્રેકર, કૂલડાઉન અને
  લોકઆઉટનું ઊંડાણપૂર્વકનું વિશ્લેષણ.
- [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — Auto Combo સ્કોરિંગ અને વ્યૂહરચનાઓ.
- [MCP-SERVER.md](../frameworks/MCP-SERVER.md) — સંપૂર્ણ MCP ટૂલ કેટલોગ + ટ્રાન્સપોર્ટ્સ.
- [A2A-SERVER.md](../frameworks/A2A-SERVER.md) — A2A પ્રોટોકોલ કૌશલ્યો અને ડિસ્કવરી.
...
- [CLAUDE.md](../../CLAUDE.md) — Claude Code માટેના રિપોઝિટરી નિયમો (ઉપરોક્ત ઘણી
  પ્રણાલીઓ માટે સત્યનો અધિકૃત સ્રોત).
- [AGENTS.md](../../AGENTS.md) — એજન્ટો દ્વારા ઉપયોગમાં લેવાતો વધુ ઊંડો આર્કિટેક્ચર સંદર્ભ.
