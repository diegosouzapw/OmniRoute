...
Reġistru: `open-sse/config/providers/registry/cursor/index.ts`
(`cursor_apiProvider`, `authType: "apikey"`, l-istess `format`, `baseUrl` u
`models` bħal `cursor`). Kard tal-katalgu:
`src/shared/constants/providers/apikey/specialty-media.ts`. Mappa tal-executor:
...
| `JWT_SECRET` mhux issettjat                              | 503 waqt l-iskambju                                   |
