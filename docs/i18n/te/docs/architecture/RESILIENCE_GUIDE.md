...
**ఉద్దేశ్యం:** అదే ప్రొవైడర్కు చెందిన ఇతర కనెక్షన్లు సేవలను కొనసాగిస్తుండగా, ఒక సమస్యాత్మక కీని దాటవేయడం.

**అమలు:**

- అందుబాటులో లేనిదిగా గుర్తించడం: `src/sse/services/auth.ts::markAccountUnavailable()`
- ఎంపిక: అదే ఫైల్లోని `getProviderCredentials*`
- కూల్డౌన్ లెక్కింపు: `open-sse/services/accountFallback.ts::checkFallbackError()`
- సెట్టింగ్లు: `src/lib/resilience/settings.ts`

**ఒక్కో కనెక్షన్కు ఫీల్డ్లు:**
...
- OAuth ప్రాథమిక వ్యవధి: 5s
- API-key ప్రాథమిక వ్యవధి: 3s
...
**యాంటీ-థండరింగ్-హెర్డ్ రక్షణ:** సమకాలీన వైఫల్యాల కారణంగా కూల్డౌన్ అతిగా పొడిగించబడకుండా లేదా `backoffLevel` రెండుసార్లు పెంచబడకుండా నిరోధిస్తుంది.

**టెర్మినల్ స్థితులు (కూల్డౌన్లు కావు):**

- `banned` — నిషేధిత-కీవర్డ్ / ఖాతా-నిషేధ గుర్తింపు ద్వారా సెట్ చేయబడుతుంది ([BAN_DETECTION](../security/BAN_DETECTION.md) చూడండి), అలాగే వరుసగా మూడు అప్స్ట్రీమ్ ప్రతి-అభ్యర్థన తిరస్కరణల ద్వారా (`request_rejected`, ఉదా. Anthropic OAuth 403 "Request not allowed" — `open-sse/services/requestRejectedStreak.ts`); ఒక్క తిరస్కరణ కేవలం కనెక్షన్ను కూల్డౌన్లో ఉంచుతుంది
- `expired` (పరిమిత పునఃప్రయత్నాల తర్వాత టెర్మినల్ స్థితికి మారుతుంది — ఎక్స్పోనెన్షియల్ బ్యాక్ఆఫ్తో `EXPIRED_RETRY_MAX = 3` — తద్వారా తాత్కాలిక OAuth లోపాలు ఖాతా శాశ్వతంగా డీయాక్టివేట్ కావడానికి ముందు స్వయంగా కోలుకోగలవు)
- `credits_exhausted`

క్రెడెన్షియల్స్ మారే వరకు లేదా ఆపరేటర్ వాటిని రీసెట్ చేసే వరకు ఇవి కొనసాగుతాయి. టెర్మినల్ స్థితులను తాత్కాలిక కూల్డౌన్ స్థితితో ఓవర్రైట్ చేయవద్దు.

**లేజీ రికవరీ:** `rateLimitedUntil` సమయం గడిచినప్పుడు, కనెక్షన్ మళ్లీ అర్హత పొందుతుంది. విజయవంతంగా ఉపయోగించినప్పుడు, `clearAccountError()` అన్ని లోప ఫీల్డ్లను క్లియర్ చేస్తుంది.
...
**ఉద్దేశ్యం:** బహుళ-టర్న్ ఏజెంట్ను (Claude Code, aider, అనుకూల ఏజెంట్లు) అభ్యర్థనలన్నింటిలో ఒకే ఖాతాపై ఉంచడం, తద్వారా ఒక్కో ఖాతాకు సెషన్ స్థితి ఉండే ప్రొవైడర్లలో ఖాతాల మధ్య కాంటెక్స్ట్ నష్టం మరియు పునరావృత కోల్డ్-స్టార్ట్ 429లను తగ్గించడం.

**అమలు:**

- TTL పరిష్కారం: `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- పిన్ ఎంపిక/సృష్టి: `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
- హెడర్ వెలికితీత (సాధారణం, ఏ ప్రొవైడర్కైనా): `src/sse/services/auth.ts::extractSessionAffinityKey()`
- నిల్వ చేయబడిన పిన్ పట్టిక: `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- సెట్టింగ్: `sessionAffinityTtlMs` (msలో గ్లోబల్ TTL, `0` డిసేబుల్ చేస్తుంది) — `src/lib/db/settings.ts`. మైగ్రేషన్ `124_generic_session_affinity_ttl.sql` ద్వారా Codexకు మాత్రమే చెందిన `codexSessionAffinityTtlMs` నుండి పేరు మార్చబడింది; ఇది మునుపు కాన్ఫిగర్ చేసిన ఏదైనా Codex TTLను కొత్త డిఫాల్ట్గా బదిలీ చేస్తుంది.

#7274కు ముందు, `codex` మినహా ప్రతి ప్రొవైడర్కు `resolveSessionAffinityTtlMs()` వెంటనే `0`తో నిష్క్రమించేది, కాబట్టి పిన్నింగ్ మెకానిజం మరియు హెడర్ వెలికితీత అప్పటికే ప్రొవైడర్-అజ్ఞేయంగా ఉన్నప్పటికీ, TTL సెట్టింగ్ (మరియు సెషన్ హెడర్లు) మరెక్కడా ప్రభావం చూపలేదు. ఆ పరిష్కారం ఆ ముందస్తు-రిటర్న్ను తొలగించింది; ఇప్పుడు TTL గ్లోబల్గా `0` కంటే ఎక్కువగా సెట్ చేసిన తర్వాత ప్రతి ప్రొవైడర్కు ఏకరీతిగా వర్తిస్తుంది.

మూడు సెషన్-అఫినిటీ హెడర్లు అప్స్ట్రీమ్కు ఎప్పటికీ ఫార్వర్డ్ చేయబడవు — ఎగ్జిక్యూటర్లు క్లయింట్ హెడర్లను యథాతథంగా పంపకుండా తమ స్వంత అప్స్ట్రీమ్ హెడర్లను మొదటి నుంచి నిర్మిస్తాయి, కాబట్టి ఇది అంతర్గత కోరిలేషన్ IDగానే ఉంటుంది.

### ప్రత్యేక మేనేజ్డ్ సెషన్ కనెక్షన్ లీజ్లు

**పరిధి:** ఒక యాక్టివ్ మేనేజ్డ్ HTTP క్లయింట్/సెషన్ ఒక అర్హతగల OmniRoute కనెక్షన్ను కలిగి ఉంటుంది.

**ఉద్దేశ్యం:** అభ్యర్థనలన్నింటిలో కఠినమైన రూటింగ్
హద్దు అవసరమైన క్లయింట్లకు దీర్ఘకాలిక ప్రత్యేక కనెక్షన్ యాజమాన్యాన్ని అందించడం. ఇది మృదువైన కొనసాగింపు ప్రాధాన్యత అయిన సెషన్ అఫినిటీకి భిన్నంగా ఉంటుంది:
ప్రత్యేక లీజ్ SQLiteలో లైఫ్సైకిల్ స్థితిని కొనసాగిస్తుంది, గ్లోబల్ యాక్టివ్-ఓనర్ మరియు
యాక్టివ్-కనెక్షన్ ప్రత్యేకతను అమలు చేస్తుంది, అలాగే ప్రొవైడర్ డిస్పాచ్కు ముందు పాత జనరేషన్ను తిరస్కరిస్తుంది.

ఈ ఫీచర్ను ఒక్కో API కీకి ఐచ్ఛికంగా ప్రారంభించాలి. మేనేజ్డ్ కీకి `lease:exclusive` స్కోప్ మరియు
స్పష్టమైన, ఖాళీ-కాని `allowedConnections` జాబితా ఉండాలి. ఏ HTTP క్లయింట్ అయినా లైఫ్సైకిల్ ఎండ్పాయింట్ను ఉపయోగించవచ్చు; ఏ
క్లయింట్ పేరు, user-agent, ప్రొవైడర్, OAuth పద్ధతి లేదా మోడల్ అవసరం లేదు. లీజ్ ఒక కనెక్షన్ను కలిగి ఉంటుంది,
మోడల్ను కాదు, కాబట్టి కనెక్షన్ సాధారణంగా
అర్హతతో ఉన్నంతకాలం మోడల్ మార్పు బైండింగ్ను అలాగే ఉంచుతుంది. సాధారణ మోడల్, కోటా, ఆరోగ్యం, కూల్డౌన్ మరియు అనుమతి జాబితా నియమాలకే
అధికారం ఉంటుంది; అవి అదే జనరేషన్ను మరొక ఖాళీ అర్హతగల కనెక్షన్కు మార్చవచ్చు.

లైఫ్సైకిల్ అనేది `acquire`, `renew`, మరియు `release` JSON చర్యలతో కూడిన `POST /api/v1/session-leases`.
మేనేజ్డ్ ఇన్ఫరెన్స్ అభ్యర్థనలు అస్పష్టమైన `X-OmniRoute-Lease-Owner` విలువను మరియు ఖచ్చితమైన
`X-OmniRoute-Lease-Generation`ను అందిస్తాయి. ఓనర్లో `vlo_` తర్వాత 43 base64url అక్షరాలు ఉంటాయి; దాని
SHA-256 హ్యాష్ మాత్రమే నిల్వ చేయబడుతుంది. ప్రతి తుది డిస్పాచ్ హద్దు కూడా ప్రమాణీకరించిన API కీ IDని మరియు
యాక్టివ్ కనెక్షన్ IDని బైండ్ చేస్తుంది. లీజ్ కంట్రోల్ హెడర్లు లాగ్లు, నిల్వ ఉంచిన అభ్యర్థన స్నాప్షాట్లు మరియు
అప్స్ట్రీమ్ ఎగ్జిక్యూటర్ హెడర్ల నుంచి తొలగించబడతాయి.

సాధారణ రూటింగ్లో అర్హతగల మేనేజ్డ్ అభ్యర్థులు ఉన్నప్పటికీ ప్రతి ఖాళీ అభ్యర్థి ఒక
వేరే యాక్టివ్ లీజ్ ఆధీనంలో ఉంటే, OmniRoute HTTP `429`, lease-capacity-unavailable కోడ్, ఒక
కెపాసిటీ కోసం-వేచి-ఉండే స్థితి, మరియు సంబంధిత అత్యంత సమీప గడువు ముగింపు నుంచి ఉత్పన్నమైన పరిమిత `Retry-After`ను తిరిగి ఇస్తుంది.
సాధారణంగా అర్హత ఖాళీగా ఉండటం లీజ్ పోటీ కాదు మరియు దాని ప్రస్తుత రూటింగ్ లోప సెమాంటిక్స్ను కొనసాగిస్తుంది.

సంబంధిత మెకానిజమ్లు వేర్వేరుగా ఉంటాయి:

- OAuth సెషన్ ఆక్యుపెన్సీ అనేది OAuth ఖాతాల కోసం ప్రాసెస్-లోకల్ మృదువైన పంపిణీ.
- ఖాతా సెమాఫోర్లు అభ్యర్థన-సమకాలీనత అనుమతులను మంజూరు చేస్తాయి మరియు అభ్యర్థన పూర్తయినప్పుడు ముగుస్తాయి.
- ప్రత్యేక మేనేజ్డ్ సెషన్ లీజ్లు అనేవి జనరేషన్ హద్దుతో కూడిన దీర్ఘకాలిక లైఫ్సైకిల్ యాజమాన్యం.
...
## 5. అభ్యర్థన క్యూ అడ్మిషన్ నియంత్రణ (v3.8.49 · సమస్య #6593)

**పరిధి**: పైన పేర్కొన్న మూడు మెకానిజమ్లకు ఒక లేయర్ దిగువన ఉండే, స్థానిక ప్రతి-ప్రొవైడర్+కనెక్షన్
రేట్-లిమిట్ క్యూ (`open-sse/services/rateLimitManager.ts`, Bottleneck ఆధారితమైనది).

**`maxWaitMs` అనేది ఎగ్జిక్యూషన్ గడువు ముగింపుకు సంబంధించిన పాత పర్సిస్టెడ్ పేరు.**
`resilienceSettings.requestQueue.maxWaitMs`, Bottleneckకు జాబ్ `expiration`గా
పంపబడుతుంది; దీని టైమర్ డిస్పాచ్ తర్వాత మాత్రమే ప్రారంభమవుతుంది. అందువల్ల ఇది స్థానిక క్యూలో
గడిపిన సమయాన్ని కాకుండా, లిమిటర్ నిర్వహించే ఎగ్జిక్యూషన్ను పరిమితం చేస్తుంది. గడువు ముగింపు
విశ్వసనీయ స్థానిక `code: "RATE_LIMIT_EXECUTION_TIMEOUT"` (HTTP 504)గా
బహిర్గతమవుతుంది; మునుపటి క్యూ-టైమ్అవుట్ కోడ్ పేరు విశ్వసనీయ అంతర్గత బ్యాక్వర్డ్
కంపాటిబిలిటీ కోసం మాత్రమే అంగీకరించబడుతుంది. డిఫాల్ట్ 15000ms; దీన్ని
`RATE_LIMIT_MAX_WAIT_MS` (env) లేదా డ్యాష్బోర్డ్ (**Settings → Resilience**,
1–30000ms UI గరిష్ఠ పరిమితి) ద్వారా ఓవర్రైడ్ చేయండి. క్యూలో ఉండే సమయానికి ఎలాంటి
కాలపరిమితి లేదు; క్యూలో ఉన్న కాలర్లను పరిమితం చేయడానికి దిగువన ఉన్న
`maxQueueDepth`ను ఉపయోగించండి.

**`maxQueueDepth` — ఆప్ట్-ఇన్ అడ్మిషన్ పరిమితి (కొత్తది).** ఒక ప్రొవైడర్+కనెక్షన్ కోసం
ఒకేసారి క్యూలో (ఇంకా డిస్పాచ్ కాని) ఎన్ని అభ్యర్థనలు ఉండవచ్చో
`resilienceSettings.requestQueue.maxQueueDepth` పరిమితం చేస్తుంది. క్యూ ఇప్పటికే
`maxQueueDepth` అభ్యర్థనలను కలిగి ఉన్నప్పుడు, కొత్త అభ్యర్థన `limiter.schedule()`కు
చేరకముందే టైప్డ్ `code: "RATE_LIMIT_QUEUE_FULL"` లోపంతో వెంటనే తిరస్కరించబడుతుంది
— కాబట్టి తిరస్కరణ తక్కువ ఖర్చుతో జరుగుతుంది మరియు ఆ అభ్యర్థన కోసం ఏదైనా డౌన్స్ట్రీమ్
ప్రాంప్ట్-కంప్రెషన్ / అనువాద పని జరగకముందే సంభవిస్తుంది. డిఫాల్ట్ `0` =
నిలిపివేయబడింది, తద్వారా ఇప్పటికే ఉన్న అపరిమిత-క్యూ ప్రవర్తన కొనసాగుతుంది; పరిమితి
0–100000. దీన్ని `RATE_LIMIT_MAX_QUEUE_DEPTH` (env) లేదా
`resilienceSettings.requestQueue.maxQueueDepth` (డ్యాష్బోర్డ్/API ప్యాచ్) ద్వారా
ఓవర్రైడ్ చేయండి.

అడ్మిషన్ తనిఖీ స్వయంగా ఒక ప్యూర్ ఫంక్షన్
(`open-sse/services/rateLimitManager/admission.ts::checkQueueAdmission`), కాబట్టి
వాస్తవ Bottleneck లిమిటర్ లేకుండానే దీనిని యూనిట్-టెస్ట్ చేయవచ్చు.

> #6593ను ప్రారంభించిన RFC, `bypassCompressionOnRateLimit` ఫ్లాగ్ను కూడా
> ప్రతిపాదించింది. ఈ రెపోలోని `open-sse/services/compression/` పైప్లైన్ అనేది
> అవుట్బౌండ్ LLM అభ్యర్థనపై (`chatCore.ts`, `resolveCompressionSettings`/
> `selectCompressionStrategy` బ్లాక్ చుట్టూ) జరిగే ప్రాంప్ట్/కాంటెక్స్ట్ కంప్రెషన్,
> సింథసైజ్ చేసిన 429 బాడీలపై జరిగే HTTP రెస్పాన్స్ కంప్రెషన్ కాదు — కాబట్టి లిటరల్
> బైపాస్ ఫ్లాగ్కు సరిపోలే కోడ్ పాత్ లేదు. ఆ ప్రాంప్ట్-కంప్రెషన్ దశ ప్రస్తుతం అభ్యర్థన
> పైప్లైన్లో `withRateLimit()`కు _ముందే_ అమలవుతుంది; అందువల్ల క్యూ-ఫుల్ తిరస్కరణపై
> దాన్ని దాటవేయడానికి క్రమాన్ని మార్చడం అనేది ఈ సమస్య పరిధికంటే వేరైన, పెద్ద మార్పు.
> అందుకే ఇది ఉద్దేశపూర్వకంగా ఇక్కడ అమలు **చేయబడలేదు**; CPU ఆదా వల్ల కలిగే ప్రయోజనం,
> క్రమాన్ని మార్చడం వల్ల వచ్చే ప్రమాదానికి తగినదైతే దీన్ని తదుపరి పనిగా వదిలారు.
