...
`src/shared/schemas/memory.ts`లోని `MemorySettingsExtended`లో తొమ్మిది ఎంబెడ్డింగ్ మరియు వెక్టర్ ఫీల్డ్లు అందుబాటులో ఉన్నాయి; ఇవి `src/lib/db/settings.ts` ద్వారా నిల్వ చేయబడతాయి:

| ఫీల్డ్                   | రకం                                                | డిఫాల్ట్ | వివరణ                                                                      |
| ------------------------ | -------------------------------------------------- | -------- | -------------------------------------------------------------------------- |
| `embeddingSource`        | `"remote" \| "static" \| "transformers" \| "auto"` | `"auto"` | ఉపయోగించాల్సిన ఎంబెడ్డింగ్ మూలం                                            |
...
| `customModelId`          | `string \| null`                                   | `null`   | అనుకూల ఎండ్పాయింట్కు పంపే మోడల్ ID                                         |
| `transformersEnabled`    | `boolean`                                          | `false`  | Transformers.js కోసం ఎంపిక చేసి ప్రారంభించడం (MiniLM, ~400MB)              |
| `staticEnabled`          | `boolean`                                          | `false`  | స్థానిక static potion-base-8M మోడల్ కోసం ఎంపిక చేసి ప్రారంభించడం           |
| `rerankEnabled`          | `boolean`                                          | `false`  | పునఃర్యాంకింగ్ దశను ప్రారంభించడం (ప్రతి అభ్యర్థనకు +200-500ms జోడిస్తుంది) |
| `rerankProviderModel`    | `string \| null`                                   | `null`   | `provider/model` ఆకృతిలో పునఃర్యాంక్ ప్రొవైడర్/మోడల్                       |
...
`remote` మూలం కోసం, Memory ఐచ్ఛిక `customBaseUrl` మరియు
`customModelId` సెట్టింగ్లను కూడా అంగీకరిస్తుంది. ఇవి రెండూ కలిసి గ్లోబల్ ఎంబెడ్డింగ్ రిజిస్ట్రీని మార్చకుండా OpenAI-అనుకూల `/embeddings`
ఎండ్పాయింట్ మరియు మోడల్ను ఎంచుకుంటాయి. ఉపయోగించే ముందు ఎండ్పాయింట్
సాధారణీకరించబడుతుంది మరియు ప్రొవైడర్ అవుట్బౌండ్ URL విధానం ద్వారా తనిఖీ చేయబడుతుంది: HTTP(S)
తప్పనిసరి, పొందుపరిచిన ఆధారాలు మరియు క్వెరీ స్ట్రింగ్లు తిరస్కరించబడతాయి, అలాగే క్లౌడ్-మెటాడేటా
చిరునామాలు బ్లాక్ చేయబడినవిగానే ఉంటాయి. ఖాళీ విలువలు ఎంచుకున్న రిజిస్ట్రీ ప్రొవైడర్ను అలాగే ఉంచుతాయి. డ్యాష్బోర్డ్కు
తిరిగి పంపే దోషాలు సున్నితమైన సమాచారాన్ని తొలగించి శుద్ధి చేయబడతాయి మరియు ఎండ్పాయింట్ ఆధారాలు ఎప్పుడూ లాగ్ చేయబడవు.

> **TODO (D20):** అన్ని API కీల మధ్య మెమరీలను పంచుకునే `global` స్కోప్ ఈ విడుదలలో
> అమలు చేయబడలేదు. దీనికి స్కీమా మార్పులు మరియు గ్లోబల్ రిట్రీవల్
> మార్గం అవసరం. దీన్ని విడిగా ట్రాక్ చేయండి.
