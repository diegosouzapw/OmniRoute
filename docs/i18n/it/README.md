...
>
> <sub>Questi dati vengono sottoposti nuovamente a verifica ogni due settimane confrontandoli con il catalogo live e **possono variare in entrambe le direzioni**: se un provider termina un piano gratuito, il numero diminuisce; se ne arriva uno nuovo, aumenta. Pubblichiamo ciò che il catalogo calcola effettivamente, mai una stima ottimistica arrotondata per eccesso.</sub>
...
## 🤖 CLI e agenti di programmazione compatibili

> Un'unica configurazione — `http://localhost:20128/v1` — e **qualsiasi** IDE o CLI con IA funziona con modelli gratuiti e a basso costo.
...
<sub>📖 Configurazione specifica per tutti i 36 strumenti (26 CLI di codice + 10 agenti CLI) → [`docs/reference/CLI-TOOLS.md`](docs/reference/CLI-TOOLS.md) · 🧩 Plugin OpenCode → [`@omniroute/opencode-provider`](https://www.npmjs.com/package/@omniroute/opencode-provider)</sub>
...
le credenziali vengono iniettate per ogni processo e Qwen/Gemini ricevono una home isolata temporanea:
...
# Oppure scegli interattivamente provider+modello e scrivi la configurazione dello strumento:
omniroute configure codex          # anche: claude opencode qwen aider goose gemini cline continue kilo
```

Ogni comando rispetta il contesto remoto attivo (`omniroute connect <host>`), `--dry-run`
mostra in anteprima le variabili d'ambiente/gli argomenti esatti senza eseguire nulla, mentre `--api-key-env NAME` evita
...
> **357 provider registrati** nelle raccolte canoniche di chat, contenuti multimediali, ricerca, servizi locali, agenti cloud e sistema, inclusi **152 con metadati di individuazione `hasFree: true`**. Il registro dei modelli di chat comprende **229 provider / 2.554 coppie provider-modello distinte / 1.283 ID modello grezzi**; il catalogo separato dei budget gratuiti contiene **491 righe per modello**, **35 pool ricorrenti** e **54 provider gratuiti per sempre, ricorrenti o senza chiave**. Questi denominatori sono intenzionalmente diversi; le definizioni e i calcoli con deduplicazione dei pool sono disponibili nella [Guida di riferimento dei provider](docs/reference/PROVIDER_REFERENCE.md) e nei [Piani gratuiti](docs/reference/FREE_TIERS.md).
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="150"><img src="./public/openference.svg" width="42" alt="Openference"/><br/><b>Openference</b><br/><sub>Qwen3.8 27B, Llama 3.2<br/>Piano gratuito</sub></td>
...
<table>
  ...
  <tr><td nowrap><b>Autenticazione e sicurezza</b></td><td>OAuth 2.0 (PKCE) + JWT + chiavi API + autenticazione MCP con ambiti · AES-256-GCM per i dati a riposo · DOMPurify</td></tr>
  <tr><td nowrap><b>Stealth</b></td><td>wreq-js — impersonificazione delle impronte digitali TLS JA3 / JA4, proxy a 3 livelli</td></tr>
  <tr><td nowrap><b>Resilienza</b></td><td>Circuit breaker, backoff esponenziale, protezione anti-thundering-herd, auto-riparazione automatica delle combinazioni</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/guides/REMOTE-MODE.md">Modalità remota</a></b></td><td>Controlla un'istanza OmniRoute remota (VPS) dalla CLI del tuo laptop tramite token di accesso con ambito limitato</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_GUIDE.md">Guida alla compressione</a></b></td><td>Pipeline a 7 opzioni: disattivata / leggera / standard / aggressiva / ultra / RTK / concatenata</td></tr>
  <tr><td nowrap><b><a href="docs/compression/RTK_COMPRESSION.md">Compressione RTK</a></b></td><td>Compressione dell'output dei comandi, filtri, attendibilità, verifica, recupero dell'output grezzo</td></tr>
  <tr><td nowrap><b><a href="docs/compression/COMPRESSION_ENGINES.md">Motori di compressione</a></b></td><td>Caveman, RTK, pipeline concatenate, interfacce dashboard/API/MCP</td></tr>
  ...
  <tr><td nowrap><b><a href="docs/reference/FREE_TIERS.md">Livelli gratuiti</a></b></td><td>Directory consolidata: 35 pool ricorrenti documentati / 489 voci di livello gratuito catalogate</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="docs/ops/RELEASE_CHECKLIST.md">Lista di controllo per il rilascio</a></b></td><td>Passaggi di convalida preliminari al rilascio</td></tr>
...
> OmniRoute è plasmato da un'appassionata comunità open source. Queste persone hanno apportato contributi eccezionali che incidono direttamente sulla qualità, sulla stabilità e sulla portata del progetto. **Grazie.**
...
<sub>Dati congelati all'ultimo commit del ramo attivo <code>release/v3.8.50</code>, <code>dafb4ae808</code>, con merge fino al 2026-08-24 05:26:03 UTC. Il censimento paginato tramite GitHub GraphQL contiene 5.911 PR unite: 2.707 del proprietario del repository, 179 di Dependabot e <b>3.025 PR esterne da 535 contributori distinti</b>. Le «righe modificate» corrispondono alla somma di aggiunte ed eliminazioni su GitHub e includono file generati, lockfile, cataloghi, traduzioni e documentazione; rappresentano il volume delle modifiche, non le righe di codice scritte. Le posizioni ex aequo al limite della graduatoria sono mantenute.</sub>
...
<table>
  <tr>
    ...
    <td align="center" width="160">
      ...
      <sub>🏅 69 commit attribuiti da GitHub · a pari merito al 6º posto</sub>
    </td>
  </tr>
  <tr>
    <td align="center" width="160">
    ...
    <td align="center" width="160">
      ...
      <sub>🏅 51 commit attribuiti da GitHub · a pari merito al 10º posto</sub>
    </td>
    <td align="center" width="160">
...
<sub>Ricontrollato il 2026-08-24 alle 06:14:31 UTC: commit attribuiti da GitHub e segnalati dall'API Contributors del repository per il branch predefinito <code>release/v3.8.50</code>. L'API ha restituito 525 identità (415 utenti, 2 bot, 108 anonime); questa tabella esclude il maintainer, i bot e le identità anonime e mantiene le posizioni a pari merito. È distinta sia dalla classifica delle PR unite riportata sopra, sia dal censimento dei metadati Git relativo a 639 persone riportato sotto.</sub>

> 🙏 Le funzionalità, le correzioni di bug e i miglioramenti all'infrastruttura apportati da queste persone sono una **parte fondamentale** di ciò che rende OmniRoute affidabile e ricco di funzionalità. Ogni pull request, ogni caso di test e ogni file di traduzione i18n è importante. L'open source è costruito da persone come loro.
