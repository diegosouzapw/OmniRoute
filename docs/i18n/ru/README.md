...
<img src="./docs/diagrams/readme-hero.svg" width="100%" alt="OmniRoute — Никогда не прекращайте кодировать. Каждый инструмент ИИ → 359 провайдеров — 150+ бесплатных — через одну конечную точку. Claude Code, Codex, Cursor, Cline, Copilot & Antigravity в БЕСПЛАТНЫЙ Claude / GPT / Gemini с автоматическим переключением. Сжатие RTK + Caveman экономит 15–95% токенов (~89% в среднем) — никогда не достигайте лимитов. 359 провайдеров ИИ · 150+ бесплатных тарифов · ~1.62B бесплатных токенов/мес · 19 стратегий маршрутизации · $0 для начала."/>
...
## 🆓 Работает сразу после установки — без ключей, без настроек

</div>

<img src="./docs/diagrams/works-zero-config.svg" width="100%" alt="Работает сразу после установки — без настроек. Три шага: 1. Установите — npm i -g omniroute, сервер запускается на localhost:20128. 2. Направьте свой инструмент на http://localhost:20128/v1 — любой инструмент, совместимый с OpenAI (Claude Code, Cursor, Cline). 3. Он отвечает — вызовите модель auto для мгновенного ответа, без ключа API, без регистрации, без настройки. Бесключевой провайдер OpenCode Free предварительно подключен к комбинации auto, поэтому свежая установка отвечает сразу же."/>
...
<sub>📦 Скрипты быстрого запуска для **Python, Node.js, PHP и cURL** → [`examples/quickstart/`](examples/quickstart/)</sub>
...
<img src="./docs/diagrams/promise-pillars.svg" width="100%" alt="Обещание — Одна конечная точка и 359 провайдеров. Автоматический откат продолжает маршрутизацию, пока доступна другая здоровая цель. Шесть столпов: устойчивый откат по 359 провайдерам · до 95% экономии токенов на подходящих рабочих нагрузках · $0 для начала со 150+ бесплатными уровнями и 54 постоянными/бесключевыми бесплатными провайдерами навсегда · 36 интеграций CLI/агентов через одну конфигурацию · совместимость с OpenAI, Claude, Gemini и Responses API на /v1 · производственные средства управления, включая автоматические выключатели, скрытность TLS, инструменты MCP 110, A2A, память, ограждения, оценки и 39 000+ статических тестовых объявлений по 5 100+ отслеживаемым тестовым файлам."/>
...
<img src="./docs/diagrams/why-pain-fix.svg" width="100%" alt="Почему OmniRoute — перестаньте жонглировать 10 панелями управления, неработающими ключами API и неожиданными счетами. Десять ежедневных проблем против решений: квота истекает неиспользованной → максимизируйте подписки; ограничения скорости во время кодирования → 4-уровневый автоматический откат (Подписка → API → Дешевый → Бесплатный); вывод инструментов сжигает токены → RTK + сжатие Caveman (15–95%); дорогие API → маршрутизация, оптимизированная по стоимости; каждый инструмент имеет свою собственную настройку → одна конечная точка, одна панель управления; ИИ заблокирован → 3-уровневый прокси + скрытность TLS; неработающие ключи → 3-уровневая отказоустойчивость (автоматические выключатели, охлаждение ключей, блокировка модели); команда использует одну подписку → пулы ключей с квотами справедливого распределения; запросы через чье-то облако → локальный подход с ключами, зашифрованными AES-256-GCM; отсутствие видимости расходов → живая аналитика (использование, квота, экономия, задержка p95)."/>

<div align="center">

<img src="./docs/diagrams/tier-cascade.svg" width="100%" alt="Поток запросов OmniRoute: ваша IDE или CLI (Claude Code, Cursor, Cline…) вызывает одну локальную конечную точку (http://localhost:20128/v1); интеллектуальный маршрутизатор OmniRoute (RTK + сжатие Caveman, 19 стратегий маршрутизации, автоматические выключатели, скрытность TLS, MCP, A2A, ограждения) может откатываться по 4 уровням провайдеров, пока остается подходящая здоровая цель — Уровень 1 Подписка, Уровень 2 Ключ API, Уровень 3 Дешевый и Уровень 4 Бесплатный."/>
...
<img src="./docs/diagrams/comparison-table.svg" width="100%" alt="Что отличает OmniRoute — актуальный снимок функций по сравнению с 9router, OpenRouter, CLIProxyAPI и LiteLLM по 13 возможностям. OmniRoute: 359 провайдеров, более 150 встроенных бесплатных тарифов, 19 стратегий маршрутизации, 12-движковая компрессия токенов, встроенный MCP-сервер со 110 инструментами, протокол агентов A2A, постоянная память, защитные механизмы, облачные агенты, скрытие отпечатков TLS, поддержка Desktop/Termux/PWA и 42 локали пользовательского интерфейса. OmniRoute имеет лицензию MIT и может быть развернут самостоятельно. Возможности и количество конкурентов могут меняться; см. связанную методологию."/>

<sub>📊 Полная методология и подробности по функциям по сравнению с 9router, OpenRouter, CLIProxyAPI и LiteLLM → [`docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md`](docs/comparison/OMNIROUTE_VS_ALTERNATIVES.md)</sub>
