...
<img src="./docs/diagrams/readme-hero.svg" width="100%" alt="OmniRoute — Никога не спирайте да кодирате. Всеки AI инструмент → 359 доставчици — 150+ безплатни — чрез една крайна точка. Claude Code, Codex, Cursor, Cline, Copilot & Antigravity във БЕЗПЛАТНИ Claude / GPT / Gemini с автоматично превключване. RTK + Caveman стекова компресия спестява 15–95% токени (~89% средно) — никога не достигайте лимити. 359 AI доставчици · 150+ безплатни нива · ~1.62B безплатни токени/месец · 19 стратегии за маршрутизиране · $0 за стартиране."/>
...
## 🆓 Работи веднага след инсталация — без ключове, без конфигурация

</div>

<img src="./docs/diagrams/works-zero-config.svg" width="100%" alt="Работи веднага след инсталация — нулева конфигурация. Три стъпки: 1. Инсталирайте — npm i -g omniroute, сървърът стартира на localhost:20128. 2. Насочете инструмента си към http://localhost:20128/v1 — всеки съвместим с OpenAI инструмент (Claude Code, Cursor, Cline). 3. Отговаря — извикайте модел auto за незабавен отговор, без API ключ, без регистрация, без конфигурация. Безключовият доставчик OpenCode Free е предварително свързан с auto комбото, така че свежа инсталация отговаря веднага."/>
...
<img src="./docs/diagrams/promise-pillars.svg" width="100%" alt="Обещанието — Една крайна точка и 359 доставчика. Автоматичното превключване поддържа маршрутизация, докато е налице друга здрава цел. Шест стълба: устойчиво превключване между 359 доставчика · до 95% спестяване на токени при подходящи натоварвания · $0 за стартиране със 150+ безплатни нива и 54 повтарящи се/безключови безплатни завинаги доставчика · 36 CLI/агентни интеграции чрез една конфигурация · съвместимост с OpenAI, Claude, Gemini и Responses API на /v1 · производствени контроли, включително прекъсвачи, TLS стелт, MCP 110 инструменти, A2A, памет, предпазни мерки, оценки и 39 000+ статични тестови декларации в над 5 100 проследявани тестови файла."/>
...
<img src="./docs/diagrams/why-pain-fix.svg" width="100%" alt="Защо OmniRoute — спрете да жонглирате с 10 табла за управление, мъртви API ключове и изненадващи сметки. Десет ежедневни болки срещу решения: квота изтича неизползвана → максимизирайте абонаментите; ограничения на скоростта по време на кодиране → 4-степенно автоматично превключване (Абонамент → API → Евтин → Безплатен); изходни данни от инструменти изгарят токени → RTK + Caveman компресия (15–95%); скъпи API → маршрутизация, оптимизирана за разходи; всеки инструмент със собствена настройка → една крайна точка, едно табло за управление; AI блокиран → 3-степенен прокси + TLS стелт; мъртви ключове → 3-слойна устойчивост (прекъсвачи, охлаждане на ключове, заключване на модел); екип, споделящ един абонамент → пулове от ключове със справедливи квоти; подкани през облака на някого → първо локално с AES-256-GCM криптирани ключове; липса на видимост на разходите → анализи на живо (използване, квота, спестявания, p95 латентност)."/>

<div align="center">

<img src="./docs/diagrams/tier-cascade.svg" width="100%" alt="Поток на заявките на OmniRoute: вашето IDE или CLI (Claude Code, Cursor, Cline…) извиква една локална крайна точка (http://localhost:20128/v1); интелигентният рутер на OmniRoute (RTK + Caveman компресия, 19 стратегии за маршрутизация, прекъсвачи, TLS стелт, MCP, A2A, предпазни мерки) може да превключва между 4 нива на доставчици, докато остава налична подходяща здрава цел — Ниво 1 Абонамент, Ниво 2 API ключ, Ниво 3 Евтин и Ниво 4 Безплатен."/>
...
<img src="./docs/diagrams/comparison-table.svg" width="100%" alt="Какво отличава OmniRoute — моментна снимка на функции спрямо 9router, OpenRouter, CLIProxyAPI и LiteLLM по 13 възможности. OmniRoute: 359 доставчици, 150+ вградени безплатни нива, 19 стратегии за маршрутизиране, 12-двигателна компресия на токени, вграден MCP сървър със 110 инструмента, A2A агентски протокол, постоянна памет, предпазни мерки, облачни агенти, TLS fingerprint stealth, Desktop/Termux/PWA и 42 i18n UI локали. OmniRoute е с MIT лиценз и може да се хоства самостоятелно. Възможностите и броят на конкурентите може да се променят; вижте свързаната методология."/>
...
<table>
  ...
  <tr><td nowrap><b>База данни</b></td><td>better-sqlite3 (SQLite, журнал в режим WAL) + LowDB (наследен JSON формат) — 122 домейн модула, 182 миграции</td></tr>
  <tr><td nowrap><b>Памет</b></td><td>Пълнотекстово търсене със SQLite FTS5 + int8-квантувани векторни представяния, типизирано затихване</td></tr>
  ...
  <tr><td nowrap><b>Удостоверяване и сигурност</b></td><td>OAuth 2.0 (PKCE) + JWT + API ключове + удостоверяване с обхвати за MCP · AES-256-GCM за данни в покой · DOMPurify</td></tr>
  <tr><td nowrap><b>Прикриване</b></td><td>wreq-js — имитиране на JA3 / JA4 TLS отпечатъци, 3-нивов прокси механизъм</td></tr>
  <tr><td nowrap><b>Устойчивост</b></td><td>Прекъсвач на веригата, експоненциално изчакване, защита срещу лавинообразни едновременни заявки, самовъзстановяване с автоматично комбиниране</td></tr>
  <tr><td nowrap><b>Журнализиране</b></td><td>pino — структурирани JSON журнали с контекст на заявката</td></tr>
  <tr><td nowrap><b>Тестване</b></td><td>Инструмент за изпълнение на тестове на Node.js + Vitest — <b>39 000+ статични декларации на тестове</b> в 5 100+ проследявани тестови файла (модулни, интеграционни, E2E, за сигурност, за екосистемата)</td></tr>
