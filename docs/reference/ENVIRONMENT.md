...
| `QUOTA_PREFLIGHT_CUTOFF_ENABLED`           | `false`                                                                     | `src/lib/resilience/settings.ts`                                          | Opt-in hard auto-routing quota cutoff. When enabled (`true`, `1`, or `on`), low-quota candidates are dropped before scoring; default is off so soft quota penalty and cooldown behavior remain unchanged.                                                                                  |
