...
| `LIVE_WS_ALLOWED_HOSTS`                     | _(unset)_                       | `src/server/ws/liveServer.ts`                                            | Comma-separated extra `Host` header values allowed for live WebSocket upgrades. Use with `NEXT_PUBLIC_LIVE_WS_PUBLIC_URL` when exposing the live server behind a tunnel or proxy. |
| `NEXT_PUBLIC_LIVE_WS_PUBLIC_URL`            | _(unset)_                       | `src/server/ws/liveServer.ts`                                            | Browser-visible live WebSocket URL override. Set this when the dashboard should connect through a public tunnel/proxy instead of deriving the local live WS URL. |
