...
| `DEFAULT_RATE_LIMIT_PER_DAY`            | `0`                   | `src/shared/utils/apiKeyPolicy.ts`       | Fallback per-day request budget applied to API keys whose `rate_limits` column is null. `0` disables the fallback (unlimited). Any positive integer N enables N/day, 5N/week, 20N/month windows.                                                             |
