...
Special thanks to **[TOON](https://github.com/toon-format/toon)** by **[toon-format](https://github.com/toon-format)** — the Token-Oriented Object Notation project (compact, schema-aware JSON for LLM prompts) whose columnar, header-plus-rows model inspired OmniRoute's `headroom`/SmartCrusher engine: a dependency-free, lossless tabular compaction of homogeneous JSON arrays with an explicit `[N rows]` marker.
