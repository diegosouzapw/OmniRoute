 ...
 * here, keyed by the identity of the request body it was handed (the same
 * motif as `opencodeRequestShape.ts`), while the pick cursor stays on the
 * instance and shared member health lives in an instance-owned store the
 * caller passes in.
 */
import type { OpencodeAccountProxyConfig } from "./opencode.ts";
