 ...
 * adds them via `ladderOverride` — same for `codex-responses`, `ionizer`, `relevance`,
 * `llm`, and `read-lifecycle`. Placement follows each engine's documented `stackPriority` in
...
const AGGRESSIVENESS: Record<string, number> = {
  ...
  "codex-responses": 23, // stackPriority 12 — bounded Responses tool-output compression
...
const REDUCTION_FACTOR: Record<string, number> = {
  ...
  rtk: 0.85,
  "codex-responses": 0.84, // lossless-first JSON plus bounded diagnostic compression
