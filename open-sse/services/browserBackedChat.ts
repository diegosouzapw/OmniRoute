...
let testOverride:
  | ((req: BrowserBackedChatRequest) => Promise<BrowserBackedChatResult>)
  | null = null;
...
export function chatUrlMatcher(u: string, matchDomain: string, chatUrl: string): boolean {
  ...
  // All segments except one dynamic middle must match.
...
export async function browserBackedChat(
  ...
  try {
    ...
    await page.goto(chatPageUrl, { waitUntil: "domcontentloaded", timeout: 60000, signal: signal ?? undefined });
...
export async function tryBackedChat(
  ...
  try {
    ...
    if (req.cookieDomain) {
      ...
      if (freshCookie) {
        const retry = await httpBackedChat({ ...req, cookieString: freshCookie, signal: effectiveSignal ?? undefined });
    ...
    const slowReq = freshCookie ? { ...req, cookieString: freshCookie, signal: effectiveSignal ?? undefined } : { ...req, signal: effectiveSignal ?? undefined };
