...
import { getHeaderValueCaseInsensitive, resolveCompressionHeader } from "./headers.ts";
...
/**
 * Prompt compression, reactive context compaction and caveman output styling.
 *
 * Lifted verbatim out of handleChatCore. The block reads a wide slice of the
 * request state and writes back twelve values, so the contract is explicit:
 * everything it reads arrives as a parameter, everything it reassigns comes
 * back in the result and the caller reseats it. No behaviour change.
 */
type CompressionBody = Record<string, unknown>;
type PostTranslationCompressor =
...
export interface ApplyContextCompressionInput {
  ...
  preCompressionBody: CompressionBody | null;
  reactiveContextCompactionEnabled: boolean;
  runPostTranslationCompression: PostTranslationCompressor;
...
export async function applyContextCompression({
  ...
  tokensCompressed,
}: ApplyContextCompressionInput): Promise<ApplyContextCompressionResult> {
  // prettier-ignore
