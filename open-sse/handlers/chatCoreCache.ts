/**
 * Internal caches used by handleChatCore to avoid repeated DB lookups.
 *
 * Both caches live here (rather than inside chatCore.ts) so the inner
 * `handleChatCore` function can stay focused on the request pipeline.
 */

import { getUpstreamProxyConfig } from "@/lib/localDb";

// ── Upstream proxy config cache ──

const _proxyConfigCache = new Map<
  string,
  { mode: string; enabled: boolean; ts: number }
>();
const PROXY_CONFIG_CACHE_TTL = 10_000;

export async function getUpstreamProxyConfigCached(
  providerId: string
): Promise<{ mode: string; enabled: boolean }> {
