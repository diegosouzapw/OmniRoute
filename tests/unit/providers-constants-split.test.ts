...
// + helpers still exported), the spread-merge integrity (178 APIKEY entries, no loss/dup), and that
...
// to 177, then #7035 added Agnes AI, bringing the catalog to 178.
...
test("APIKEY_PROVIDERS merges the 6 family files into 178 entries (no loss / no dup)", async () => {
  const keys = Object.keys((P as Record<string, object>).APIKEY_PROVIDERS);
  assert.equal(keys.length, 178);
  assert.equal(new Set(keys).size, 178, "duplicate keys after spread-merge");
  // the merged object's entry-count equals the sum of the 6 semantic family files; families are a
  // strict partition (every provider in exactly one), so the sum must be exactly 178.
  ...
  assert.equal(famTotal, 178, "families must partition all 178 providers");
});
