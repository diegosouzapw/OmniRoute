...
const originalSandboxRuntime = process.env.SKILLS_SANDBOX_RUNTIME;

// Pin the sandbox runtime to `docker` for the existing Docker-focused tests
// so they remain deterministic regardless of the CI host's installed runtimes.
process.env.SKILLS_SANDBOX_RUNTIME = "docker";
...
test.afterEach(() => {
  ...
  // Restore runtime env var — provider detection tests may mutate it
  if (originalSandboxRuntime === undefined) {
    delete process.env.SKILLS_SANDBOX_RUNTIME;
  } else {
    process.env.SKILLS_SANDBOX_RUNTIME = originalSandboxRuntime;
  }
});
