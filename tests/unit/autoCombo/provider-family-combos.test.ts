...
describe("auto/<family> materialization (#6453)", () => {
  it("resolves auto/glm to a virtual combo spanning every connected GLM backend", async () => {
    ...
    // The upstream no-auth allowlist intentionally excludes `auggie` from every
    // auto/* pool. This family therefore spans only the connected GLM backends;
    // allowlisted no-auth providers remain eligible when they serve the family.
