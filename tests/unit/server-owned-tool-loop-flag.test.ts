...
describe("feature-flags-settings count update", () => {
  it("flag count matches updated expected value", () => {
    ...
    // (flush empty-turn retry, default off) bumps it to 76. The registry already
    // held 77 when UNPRICED_USAGE_BUDGET_POLICY (#14604) was added, taking it to 78
    // (tests/unit/feature-flags-settings.test.ts carries the same count).
