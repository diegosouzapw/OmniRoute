...
const migrationPath = path.join(migrationDir, "144_windsurf_to_devin_desktop.sql");
...
test("migration 144 for the Devin Desktop provider exists", () => {
...
test("migration 144 moves provider aliases when the destination is absent", () => {
...
test("migration 144 moves live key provisioning state with destination-wins collisions", () => {
  const migration = fs.readFileSync(migrationPath, "utf8");
...
test("migration 144 moves current settings safely and preserves historical records", () => {
  const migration = fs.readFileSync(migrationPath, "utf8");
  const db = createDb();
  db.exec(`
