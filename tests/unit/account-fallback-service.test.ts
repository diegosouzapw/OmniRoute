...
test("recordModelLockoutFailure sets cooldown until tomorrow 0:00 for quota_exhausted reason", () => {
  ...
  try {
    ...
    // Calculate expected ms until tomorrow 0:00
    ...
    assert.ok(
      result.cooldownMs >= expectedMsUntilTomorrow - tolerance &&
        result.cooldownMs <= expectedMsUntilTomorrow + tolerance,
      `cooldown should be ms until tomorrow 0:00 (expected ~${expectedMsUntilTomorrow}ms, got ${result.cooldownMs}ms)`
    );
