...
// table. Migration 186 adds idx_turn_nodes_last_seen.
...
fs.copyFileSync(
  path.join(repoMigrations, "186_turn_nodes_last_seen_index.sql"),
  path.join(migrationsDir, "186_turn_nodes_last_seen_index.sql")
);
...
function indexes(db: Database.Database): string[] {
  return (
    db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND tbl_name = 'conversation_turn_nodes'")
...
test("migration 186 creates idx_turn_nodes_last_seen and a second run is a no-op", () => {
  ...
  try {
    ...
    assert.deepEqual(db.prepare("SELECT version, name FROM _omniroute_migrations").all(), [
      { version: "186", name: "turn_nodes_last_seen_index" },
    ]);
  } finally {
    db.close();
  }
...
test("the retention predicate uses the index (no full scan)", () => {
  ...
  try {
    ...
    for (let i = 0; i < 500; i++) {
      insert.run(`n${i}`, `c${i % 10}`, `2026-09-${String((i % 28) + 1).padStart(2, "0")}T00:00:00.000Z`);
    }
    const plan = db
      .prepare(
