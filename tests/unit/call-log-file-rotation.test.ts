...
test("call log file rotation honors both retention days and file count", () => {
  ...
  // keep-a should be deleted (because max entries is 2, and we have 4 logs: old-log, keep-a, keep-b, keep-c -> keep the two most recent: keep-b, keep-c)
  assert.equal(
    db.prepare("SELECT COUNT(*) AS cnt FROM call_logs WHERE id = ?").get("keep-a").cnt,
    0
  );
  // keep-b and keep-c should be kept
  assert.equal(
    db.prepare("SELECT COUNT(*) AS cnt FROM call_logs WHERE id = ?").get("keep-b").cnt,
    1
  );
  assert.equal(
    db.prepare("SELECT COUNT(*) AS cnt FROM call_logs WHERE id = ?").get("keep-c").cnt,
    1
  );

  // Old log file should be deleted, keep-a file should be deleted (trimmed), keep-b and keep-c files should remain
