...
// #13130: migration 180 adds call_logs.ttft_ms (dashboard generation-time TPS
// reads it via mapSummaryRow on every row). A lineage that skipped 180 must
// still expose the column, same reconciliation pattern as #12150/#12470.
test("ensureCallLogsColumns back-fills ttft_ms on a pre-180 lineage", () => {
  const db = openMemoryDb();
  try {
    db.exec("CREATE TABLE call_logs (id TEXT PRIMARY KEY, timestamp TEXT)");
