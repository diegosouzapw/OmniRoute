...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "omniroute-image-route-codex-fb-"));
process.env.DATA_DIR = TEST_DATA_DIR;
process.env.API_KEY_SECRET = process.env.API_KEY_SECRET || "image-route-codex-fb-secret";
  ...
  return imageRoute.POST(
    new Request("http://localhost/api/v1/images/generations", {
      ...
      body: JSON.stringify({
        ...
        prompt: "draw a codex account fallback regression guard",
...
test("v1 image generation POST skips Codex accounts that exclude the requested model", async () => {
...
test("v1 image generation POST retries one Codex sibling for the ChatGPT account model-access error", async () => {
  const unsupportedConnection = await seedConnection("codex", {
    apiKey: "unsupported-token",
    name: "codex-unsupported-account",
    priority: 1,
  });
  const supportedConnection = await seedConnection("codex", {
...
test("v1 image generation POST does not retry ordinary Codex 400 responses", async () => {
  await seedConnection("codex", {
    apiKey: "invalid-request-token",
    priority: 1,
  });
  await seedConnection("codex", {
    apiKey: "unused-token",
...
test("v1 image generation POST stops after one Codex account fallback", async () => {
  await seedConnection("codex", {
    apiKey: "unsupported-first-token",
    priority: 1,
  });
  await seedConnection("codex", {
    apiKey: "unsupported-second-token",
    priority: 2,
  });
...
test("v1 image generation POST attributes Codex call logs to the selected connection", async () => {
