...
test("default comboCooldownWait covers Gemini-class TPM/RPM waits (on, 65s ceiling, 2 attempts, 130s budget)", () => {
  const s = cloneDefaults().comboCooldownWait;
  assert.equal(s.enabled, true);
  assert.equal(s.maxWaitMs, 65000);
  assert.equal(s.maxAttempts, 2);
  assert.equal(s.budgetMs, 130000);
});
...
test("mergeResilienceSettings round-trips a partial patch", () => {
  ...
  assert.equal(merged.comboCooldownWait.budgetMs, 130000);
...
test("maxWaitMs is clamped to the 90s hard ceiling", () => {
  const merged = mergeResilienceSettings(cloneDefaults(), {
...
test("garbage values fall back to the current numbers", () => {
  ...
  assert.equal(merged.comboCooldownWait.maxWaitMs, 65000);
  assert.equal(merged.comboCooldownWait.maxAttempts, 2);
  assert.equal(merged.comboCooldownWait.budgetMs, 130000);
});
