...
test("streamMaterialize keeps tip persist and cache argument shape", () => {
  const src = fs.readFileSync(
    path.resolve(here, "../../../open-sse/handlers/chatCore/streamMaterialize.ts"),
    "utf8"
  );
  assert.match(src, /error: streamError \|\| undefined/);
  assert.equal(src.includes('typeof streamError === "string"'), false);
  assert.match(src, /body: bodyForCacheWrite as \{/);
  assert.equal(src.includes('bodyForCacheWrite && typeof bodyForCacheWrite === "object"'), false);
});
