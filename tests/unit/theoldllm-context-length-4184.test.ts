...
// with no per-model contextWindow, so getResolvedModelCapabilities resolved their context
// window to `null` and the dashboard/catalog reported no usable window. #4184
// adds an entry-level `defaultContextLength` plus per-model `capabilities.contextWindow`
...
test("#4184 per-model contextWindow overrides match each upstream window", () => {
  assert.equal(model("GPT_5_4")?.capabilities?.contextWindow, 400000, "GPT-5.4 window is 400K");
  assert.equal(
    model("gemini_3_flash")?.capabilities?.contextWindow,
    1000000,
    "Gemini 3 Flash window is 1M"
  );
  assert.equal(
    model("gemini_3_pro")?.capabilities?.contextWindow,
    1000000,
    "Gemini 3 Pro window is 1M"
  );
  for (const id of ["claude_opus_4", "claude_sonnet_4", "claude_haiku_3_5", "deepseek_v4"]) {
    assert.equal(model(id)?.capabilities?.contextWindow, 200000, `${id} window is 200K`);
  }
});

test("#4184 GPT_4o carries no explicit contextWindow (relies on defaultContextLength)", () => {
  ...
  assert.equal(model("GPT_4o")?.capabilities?.contextWindow, undefined);
});
