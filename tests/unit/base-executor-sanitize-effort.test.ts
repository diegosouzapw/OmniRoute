...
// Structural shape of the request bodies exercised in this file. The function
// under test accepts/returns `unknown` (it sanitizes arbitrary provider request
// bodies), so callers narrow with a type assertion at the call site rather than
// widening the production signature. This mirrors the three effort carriers
// documented in open-sse/executors/base/reasoningEffort.ts (top-level
// reasoning_effort, reasoning.effort, output_config.effort).
interface SanitizedEffortBody {
  model?: string;
...
test("sanitizeReasoningEffortForProvider: xiaomi-mimo preserves xhigh by default", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning_effort, "xhigh");
  assert.equal((result as SanitizedEffortBody).model, "mimo-v2.5-pro", "other fields preserved");
...
test("sanitizeReasoningEffortForProvider: explicit xhigh opt-out downgrades to high", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning_effort, "high");
  assert.ok(
...
test("sanitizeReasoningEffortForProvider: Ollama Cloud preserves max", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning_effort, "max");
  assert.equal(log.messages.length, 0);
});
...
test("sanitizeReasoningEffortForProvider: nested OpenAI reasoning max normalizes to xhigh", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning.effort, "xhigh");
  assert.equal((result as SanitizedEffortBody).reasoning.summary, "auto", "other reasoning fields preserved");
  assert.equal((result as SanitizedEffortBody).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: claude preserves max for Opus/Sonnet and downgrades Haiku", () => {
  ...
  assert.equal((sonnetResult as SanitizedEffortBody).reasoning_effort, "max");
  ...
  assert.equal((opusResult as SanitizedEffortBody).reasoning.effort, "max");
  ...
  assert.equal((haikuResult as SanitizedEffortBody).reasoning_effort, "high");
});
...
test("sanitizeReasoningEffortForProvider: explicit xhigh opt-out preserves Responses shape", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning.effort, "high");
  assert.equal((result as SanitizedEffortBody).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: mistral/devstral strips reasoning_effort entirely", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning_effort, undefined, "reasoning_effort must be stripped");
  assert.ok(
...
test("sanitizeReasoningEffortForProvider: rejecting providers strip max before normalization", () => {
  ...
  assert.equal((mistralResult as SanitizedEffortBody).reasoning_effort, undefined);
  ...
  assert.equal((githubResult as SanitizedEffortBody).reasoning_effort, "high");
  ...
  assert.equal((githubHaikuResult as SanitizedEffortBody).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: mistral/devstral strips reasoning object when only effort present", () => {
  ...
  assert.equal((result as SanitizedEffortBody).reasoning, undefined, "reasoning object dropped when emptied");
});
...
test("sanitizeReasoningEffortForProvider: mistral/devstral preserves reasoning when other fields remain", () => {
  ...
  const result = sanitizeReasoningEffortForProvider(body, "mistral", "devstral-2512", null);
  assert.deepEqual((result as SanitizedEffortBody).reasoning, { summary: "auto" });
});
...
test("sanitizeReasoningEffortForProvider: native deepseek maps xhigh → max", () => {
  ...
  assert.notEqual(result, body, "must return a new object when mutating");
  assert.equal((result as SanitizedEffortBody).reasoning_effort, "max");
  assert.equal((result as SanitizedEffortBody).model, "deepseek-v4-pro", "other fields preserved");
  assert.ok(
...
test("sanitizeReasoningEffortForProvider: native deepseek clamps low → high", () => {
  ...
  assert.notEqual(result, body, "must return a new object when mutating");
  assert.equal((result as SanitizedEffortBody).reasoning_effort, "high", "below the {high, max} floor → high");
});
...
test("sanitizeReasoningEffortForProvider: opencode-go DeepSeek V4 Pro preserves variant suffix levels", () => {
  for (const level of ["low", "medium", "high", "max"]) {
    ...
    assert.equal(
      (result as SanitizedEffortBody).reasoning_effort,
      level,
...
test("sanitizeReasoningEffortForProvider: #7044 output_config.effort (Claude native) xhigh is downgraded, not bypassed", () => {
  ...
  assert.notEqual(result, body, "must return a new object when mutating");
  assert.equal(
    (result as SanitizedEffortBody).output_config.effort,
    "high",
    "xhigh downgraded to high on the output_config carrier"
  );
  assert.ok(
    !("reasoning_effort" in (result as SanitizedEffortBody)),
