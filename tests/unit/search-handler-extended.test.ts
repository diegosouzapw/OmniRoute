...
test("handleSearch searches with Z.AI Coding Plan via MCP", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (body.method === "initialize") {
      return new Response(
        JSON.stringify({
          jsonrpc: "2.0",
          result: { protocolVersion: "2024-11-05", capabilities: {}, serverInfo: { name: "zai-mcp", version: "1.0" } },
          id: body.id,
        }),
        {
          status: 200,
  ...
  try {
    ...
    assert.equal(result.data.results[0].snippet, "Z.AI now supports web search capabilities via Coding Plan");
