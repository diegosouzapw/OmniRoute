...
test("pi-settings GET: returns 200 with valid auth", async () => {
  const res = await GET(
    await makeManagementSessionRequest("http://localhost/api/cli-tools/pi-settings")
  );
...
test("pi-settings: error responses do not leak stack traces", async () => {
  const badReq = await makeManagementSessionRequest("http://localhost/api/cli-tools/pi-settings", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: "{ bad json }",
  });
